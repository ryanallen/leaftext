// ---------------------------------------------------------------------------
// Graph view: an Obsidian-style force-directed map of how documents link to one another, rendered with PixiJS (WebGL) and laid out with d3-force. Nodes are documents; edges are resolved doc-to-doc links. The active document is the highlighted center; clicking a node opens it and keeps the map, redrawn around what opened and flown to it; hovering lights up its links.
// ---------------------------------------------------------------------------
...
// How far the world container can zoom in/out (mouse wheel and focus flights are both clamped to this). Kept as constants so the label supersample below can be tied to the same ceiling.
const GRAPH_MIN_ZOOM = 0.15;
const GRAPH_MAX_ZOOM = 4;
// Screen margin left around the graph when it fits itself to the view. Larger than a node needs, because a label hangs above each one.
const GRAPH_FIT_PADDING = 64;
// When we fly the graph to a node (clicking its tab), settle at least this zoom so the node reads as focused; never zoom out from a closer view the user set.
const GRAPH_FOCUS_ZOOM = 2.2;
const GRAPH_FOCUS_DURATION_MS = 420;
// Ambient labels (the names floating by the nodes you are not on) render at a fixed screen size and are decluttered by collision — see layoutGraphLabels.
const GRAPH_LABEL_FONT_SIZE = 11;
const GRAPH_LABEL_GAP = 4; // screen px between a node and the top of its label
// Above this node count, skip ambient labels: the collision pass would rarely place any in a dense overview and the per-relayout cost stops being free. Active/hover labels still show at any size.
const GRAPH_AMBIENT_LABEL_MAX = 400;
// Where a rebuild that inherited the last layout starts the simulation: a nudge to absorb what changed, not a fresh layout. Full alpha throws the whole map out from the center again, which is what a save looks like from the reader's side.
const GRAPH_WARM_ALPHA = 0.3;
// Arrowheads, in world units like the node radii, so a head holds its size against the circle it points at through any zoom.
const GRAPH_ARROW_LENGTH = 6;
const GRAPH_ARROW_HALF_WIDTH = 3;
// Where a head is only ink per frame: a map already a thicket, or a zoom that makes it a couple of pixels.
const GRAPH_ARROW_MAX_NODES = 800;
const GRAPH_ARROW_MIN_ZOOM = 0.5;
// A burst of writes under the vault arrives as a burst of graphs. Build the last and skip the rest — each rebuild is a WebGL context thrown away.
const GRAPH_REBUILD_COALESCE_MS = 150;
let graphRebuildTimer = 0;

// Show the map instead of the document, or put the document back. One flag for the window, not a mode each tab remembers: switching tabs moves the highlight to the new document rather than swapping the picture for another one.
...
// Leaving the graph for a document, which is what opening one means. Silent when the graph was not up.
...
// Graph size: persist the choice and, if the graph is on screen, rebuild it for the new scope right away.
...
// The tools of the view you are in. None turns blue: the filled chip means "this is the view you are in", and a setting inside that view must not wear it. The glyph carries the state instead -- a shut padlock, a thin first letter. The padlock stands in both editable views, but it is a different switch in each and its tooltip says which; the map's one tool is how big a graph to draw.
...
// The reading view's padlock. Flipping it re-renders the document, which is what binds or drops the editable blocks; any block mid-edit is committed first rather than silently discarded. The reader stays where they were — the same words are on screen either way.
...
// The source view's, which is Monaco's readOnly option. Set, not rebuilt: a re-render there would throw away the editor, its undo stack and the place in the file, and nothing about the text has changed.
...
// The host turning a padlock off for you: a new document exists to be typed into and opens in the reading view, so that one must not open locked. The source keeps its own answer.
window.leafUnlockReading = () => {
  setReadingUnlocked(true);
};
// The speed reader stays one preference for the whole app -- it is a way of reading, not a property of a document. The reading toolbar is the one place it is turned on and off; the glyph then shows which way it went.
if (speedReaderButton) {
...
function setReaderView(view) {
  ...
  // Going from the map straight to the source: hold the map until the source is ready. Dropping it first reveals the reading view, and revealing it lays out a whole document we are about to replace — which is the reading view flashing up under a spinner on the way to the code view.
  if (graphViewOpen && view === 'code' && !codeViewActive) {
    graphExitPending = true;
    toggleCodeView();
    return;
  }
  // Leaving the map for the reading view. #app is hidden while the map is up, so revealing it re-lays-out the whole document — a stall on a big file with no feedback. Hold a spinner across it, matching graph→code, instead of freezing on the map. The 'graph' owner is what lets the spinner show while the map is still up (beginReaderLoading otherwise suppresses spinners there).
  if (graphViewOpen && view === 'reading' && !codeViewActive) {
    beginReaderLoading('graph');
    // Two frames before the reveal so the spinner actually paints first — the reveal below re-lays-out the whole document in one blocking frame, and without the yield the spinner and the finished reader paint together, so it never shows (mirrors runViewRender's heavy-payload path).
  ...
  setGraphView(false);
  // Reading and code are the same document either way round, so both are the one toggle — it carries the reader's place across the swap.
...
// The key earns its place only when there are two kinds of node to tell apart. A map of documents alone explains itself.
...
function loadGraphLibs() {
  ...
  // Pixi must load before its unsafe-eval companion, which patches Pixi's shader and uniform systems to avoid `new Function` (blocked by the CSP). d3-force loads in parallel — it shares nothing with Pixi.
...
// The graph's palette, read fresh from the theme tokens. Used at build time and re-read on theme change so the canvas recolors with the rest of the app.
function graphColors() {
  return {
    ...
    // Ambient labels for the documents you are not on: the muted-foreground token (a dim gray), so they read as secondary next to the active/hover labels.
    dim: cssVarColor('--lt-muted-foreground', 0x8b95a5),
    // A web address at rest. The border token — the quietest thing the theme has a name for — because these are the edge of the map rather than its subject.
...
// The Focus scope seeds from the open document, or from the recent files when no document is open (the start screen). Other scopes ignore seeds.
function graphSeeds() {
...
// Ask the backend for the graph slice for the current scope + seeds, resetting any existing scene so the view reads "loading" until fresh data arrives.
function requestGraphData() {
  ...
  graphData = null;
  teardownGraphScene();
  // The spinner a slow document gets. Building a map is the same kind of wait, and a line of text in the corner reads as a result rather than a wait.
  setGraphStatus('');
  beginReaderLoading('graph');
  send({ command: 'getGraph', scope: graphScope, seeds });
}

// Entry point when the graph view becomes visible. Requests fresh data the first time, then either builds the scene (data already in hand) or just moves the active-node highlight (scene already built).
//
// Nothing is refused here. The host decides what the map is read off — the active vault when it holds the open document, otherwise that document, its folder and what it links to — and either way an answer comes back. Refusing for want of a vault is the one thing this view must not do: a document's links are in the document.
function showGraph() {
...
// What a payload draws: the node set with its degrees, and the wires between them. Two graphs that agree on this are the same picture, however they arrived — so the one already on screen, with the layout it settled into and wherever the reader has panned it, is the better copy of it.
...
window.leafSetGraph = (payload) => {
  ...
  // Already drawing exactly this. The host redraws for any change to the vault's text, and most of them leave the map identical.
  if (graphScene && graphScene.signature === graphSignature(graphData)) return;
  // Nothing on screen yet: the first graph of a session is what the reader is waiting for, so it must not wait behind a timer.
  if (!graphScene) {
    buildGraphScene();
    return;
  }
...
// Where the map on screen had got to: every node's place, and the camera. A redraw is nearly always a small change to a picture someone is reading, so the next scene starts from here.
function carryGraphLayout(scene) {
  ...
  return {
    positions,
    // A framing the reader took is theirs to keep across a redraw; one we chose stays ours, so a graph that grew a node still gets framed.
...
async function buildGraphScene() {
  ...
  // Build node objects d3 will mutate with x/y, plus their Pixi graphics. A node the last scene had keeps its place: d3 only seeds the ones without one, so a rebuild lands on the layout that was there plus wherever the new nodes fall.
  const nodes = data.nodes.map((n) => {
    const node = {
      path: n.path,
      label: n.label || n.path,
      // The other names the note answers to. The node keeps the file's name, so these show on hover rather than on the map.
      aliases: Array.isArray(n.aliases) ? n.aliases : [],
      degree: n.degree || 0,
      // A web address rather than one of your documents: drawn as a ring, opened in the browser. See graphColors().external and the pointerup handler.
      external: !!n.external,
    };
  ...
  const scene = {
    ...
    // Frame everything until the reader takes the wheel. A view parked at 1:1 on an arbitrary center cannot answer "how much is there": two documents sit lost in an empty field, two thousand hang off every edge. Any pan, zoom, drag or flight ends it for good — including one from before a redraw.
    autoFit: carried ? carried.autoFit : true,
    // Ambient labels wait for the layout to settle so they resolve on stable positions instead of flickering as the simulation jiggles the nodes. A carried layout is already settled, so its names do not blink out.
    settled: carried ? carried.settled : false,
    // What this scene draws, so a later delivery of the same picture can be recognized and left alone.
  ...
  for (const node of nodes) {
    ...
    //
    // A web address is a ring rather than a disc. That reads as "this is not one of your files" before you have read its name, which matters because clicking it leaves the app — and it is one shape, so a tint still carries hover and active state exactly as it does for a document.
    if (node.external) {
      const radius = graphNodeRadius(node.degree);
      gfx.circle(0, 0, radius).stroke({ width: 1.5, color: 0xffffff, alignment: 0.5 });
      // A dot at the middle, so the edge has something to end behind. Edges are drawn under the nodes, and a bare ring lets the line run on through the hollow and stop in mid-air at the center.
    ...
    gfx.on('pointerover', () => {
      ...
      // The same native tooltip the library rows, hits, and tabs use: the full document path on the canvas element under the cursor, and the other names it answers to, which the map itself never labels a node with.
  ...
  // Scale the layout to the node count. Edge drawing dominates, so large graphs paint every Nth tick, settle faster, approximate charge more coarsely, and drop the collide force once it's unaffordable.
  ...
  // Inherited a layout: run the simulation warm, so it absorbs the change rather than laying the whole vault out again under a reader's eyes.
  ...
  sim.on('tick', () => {
    ...
    if (tickCount % renderEvery === 0) {
      // Follow as it settles, so a layout expanding past the edges is not watched from behind a fixed camera — but hold still while it stays in frame.
      if (scene.autoFit) fitGraphToView(scene, true);
      renderGraphFrame(scene);
    }
  });
  sim.on('end', () => {
    // The layout has stopped moving: let ambient labels resolve on the final positions, then paint.
    scene.settled = true;
  ...
  clearReaderLoading('graph');
  applyGraphStyles();
  // d3 seeds positions before the first tick, so there is already something to frame: the map opens fitted rather than snapping into place a frame later. A camera the reader set survives the redraw instead of being overruled by it.
  if (scene.autoFit) {
  ...
  layoutGraphLabels(scene);
  renderGraphFrame(scene);
  // A rebuild triggered by a deliberate navigation (tab click/switch) flies to the active node now that its graphics exist; d3 seeds positions before the first tick, so focusGraphNode tracks it as the layout settles.
  if (graphFocusPending && graphActivePath) {
    ...
    // Surface the real failure (e.g. WebGL unavailable in this WebView) on the status line instead of hanging on "Building graph…", and log a breadcrumb.
