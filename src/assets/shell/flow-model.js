// ---------------------------------------------------------------------------
// The flowchart grammar and the graph behind it. No DOM here, not one selector: this is the half check-shell.mjs can run in Node.
//
// One table. FLOW_SHAPES, and FLOW_EDGE_LINES against FLOW_EDGE_ENDS, are read in both directions — parseFlow matches what they spell, renderFlow writes it, and the sheet's palette is generated from them. So a shape the parser cannot read can never be offered, and a new one is one row.
//
// A node has an `id`, a `shape` and its `text`; an edge runs `from` one to another, with a `line` style and a pair of `ends`. Mermaid's subject matter, said plainly — don't borrow another format's field names for it.
//
// Fail closed. parseFlow returns null on anything it does not fully understand, never a partial graph: a canvas that quietly drops the half it didn't read turns "I tidied my diagram" into lost work. A box given its own size or its own place on the page is refused by that rule alone, because nothing here matches them and a save would drop them.
//
// Refusing is not the same as saying nothing. flowRefusal walks the same text again and names the line that beat the parser, because "we can't model this" fifteen different times tells the reader nothing about their diagram.
// ---------------------------------------------------------------------------

// The shapes, keyed by the name mermaid knows each one by — the id is what gets written into the file. A row says what a shape is called, what it is for, and how it is spelled. Nothing here says what one looks like: mermaid draws the canvas, so a second drawing of the same shape could only ever be a second drawing that was wrong.
//
// A row with `open` can be drawn in brackets, and is written that way. The rest have no brackets at all — they are the shapes `A@{ shape: … }` added, and the typed form is the only way to write one. `also` is every other name mermaid answers to for the same shape; we read them all and write the short one.
const FLOW_SHAPES = [
  ...
  // Mermaid's own meanings, which are not the ones these two carried before the other forty-odd arrived: manual input is sl-rect, and this pair moves up one.
...
// A shape's opener does not always decide which shape it is — `[/x/]` and `[/x\]` open the same way — so the longest opener is tried first and the closer settles it. See takeFlowNode.
const FLOW_SHAPES_BY_OPENER = FLOW_SHAPES.filter((shape) => shape.open).sort(
  (a, b) => b.open.length - a.open.length,
);

// Every name a shape answers to, mapped to the one we write. Mermaid takes all of them; keeping the aliases readable and unwritten is what stops the file growing a second spelling of a shape it already had.
...
// The catalog behind a function: a `const` in the shell script is not reachable from check-shell.mjs and a function is.
function flowShapeCatalog() {
  return FLOW_SHAPES;
}

// A connector is a line style and a pair of ends, and mermaid spells it as the product of the two — so these are two tables, not one of twenty-one rows. A token is `head + body + tail`; the labeled form mermaid also takes is `head + labelOpen + text + labelBody + tail`. Everything below falls out:
//
//   solid  --- --> --o --x <--> o--o x--x        -- text -->
//   dotted -.- -.-> -.-o -.-x <-.-> o-.-o x-.-x  -. text .->
//   thick  === ==> ==o ==x <==> o==o x==x        == text ==>
//
// A connector can also be *stretched*, and mermaid reads the extra length as a rank hint: `--->` pushes the box it points at one rank further down than `-->` would. `unit` is the character that repeats, `least` is how many of them the shortest spelling has, and `open`/`shut` are what sits either side of the run. So `----> ` is the arrow with two more units, and that number is kept on the edge — dropping it would quietly redraw the whole layout on the next save.
//
// The invisible link is a fourth row that takes no ends at all: `~~~` and nothing else. `only` says so, and the product below skips every other pair.
...
// One row covers both ends because mermaid only spells the symmetric pairs: there is no way to write a circle at one end and a cross at the other.
...
// The end that is drawn at each tip, from how it is spelled. One place, so the canvas cannot disagree with the grammar about what `o--x` would even mean.
...
// One connector, spelled. `stretch` is how many units past the shortest form it runs — 0 for `-->`, 1 for `--->`.
...
// Every spelling, as a pattern rather than a string, because the run in the middle has no fixed length. Matched longest fixed part first — `-.->` must be tried before `-.-`, or a dotted arrow reads as a dotted line with a stray `>`.
...
// The same product again, for the form that carries its label between the dashes. Parsed but never written: the canvas writes `|"label"|`, which every line style takes. The label may not hold the characters a closer starts with, or `A ----> B` would read as a labeled arrow.
...
// `graph` is the older keyword for the same diagram; both parse, and we always write `flowchart`. TB is TD under another name — kept on read, never written.
const FLOW_HEADER_RE = /^[ \t]*(?:flowchart|graph)(?:[ \t]+(TD|TB|BT|LR|RL))?[ \t]*$/i;
// A comment, and the accessibility lines. Neither attaches to anything structural, so both ride through a save untouched.
const FLOW_COMMENT_RE = /^[ \t]*%%/;
const FLOW_ACC_RE = /^[ \t]*acc(?:Title|Descr)[ \t]*:/;
// A hyphen is allowed inside an id, but only with a letter or digit behind it: that is the whole difference between `read-file` and the `A --> B` whose arrow starts one character later. `-.` is excluded for the same reason, or `A-.->B` would read as an id called `A-.`.
const FLOW_ID_RE = /^[A-Za-z0-9_][A-Za-z0-9_.]*(?:-[A-Za-z0-9_][A-Za-z0-9_.]*)*/;
// How a diagram is painted. The canvas sets none of these — it has no color picker and is not getting one — but a diagram that carries them is still a diagram to draw in, so they are read onto the thing they paint and written back off it. On the node and the edge rather than in a list of lines, which is what makes deleting a box take its color with it instead of leaving a rule pointing at nothing.
...
// Where a box goes when it is clicked: `click A "…"`, with `href` optional and a second string the tooltip. Both spellings reach the same anchor in the page, so the short one is what gets written back.
...
// A line's own name, and the one thing a named line is for. `@{` is excluded because that spelling is a box being given a shape, not a line being named.
...
// A box around boxes. `subgraph one`, `subgraph one [Title]`, `subgraph "Title"` — the id comes first when there are two, and is the title when there is one. Which group a box is in rides on the box, so moving a box among its neighbors cannot quietly move it out of its group.
...
const FLOW_INDENT = '    ';
// Mermaid's escape for a quote inside a quoted label — the one entity we both read and write, so a label with a quote in it survives the round trip.
const FLOW_QUOTE_ENTITY = '#quot;';

// What the insert row opens on: one node, so the canvas has something to work from. Written the way renderFlow writes, so it round-trips like the rest.
...
// An unquoted label. The exclusions are not fussiness: every one of these characters is a shape or an edge we do not model yet, and reading `A[/x/]` as a rectangle labeled "/x/" would silently turn a parallelogram into a box.
...
// The label inside a shape's brackets, up to `close`. Quoted or bare. Backticks inside a quoted label are mermaid's markdown string — they are the label's own text as far as we are concerned, kept whole and written back whole, so a bold word survives a save even though the canvas shows it as it was typed.
...
// `shape: cyl, label: "one, two"` in two, on the comma that is not inside the label.
...
// The typed form: `A@{ shape: cyl, label: "Cache", icon: "leaf:back" }`. Four keys are read; a size or a position is a layout we do not keep, and anything else in the braces refuses the diagram rather than being dropped on the next save.
function takeFlowTyped(rest) {
  if (!rest.startsWith('@{')) return null;
  // Scanned rather than searched for: a label may hold a brace or a comma, and both of those are what the braces and the commas out here are made of.
  let close = -1;
  let quoted = false;
  ...
  for (const part of flowTypedParts(body)) {
    ...
    if (key === 'label') {
      // A label written bare here may hold anything a bracket label may not, except the comma that would have ended it. Quoted, it may hold that too.
      if (!quoted && !flowBareLabelOk(value) && value !== '') return null;
      typed.text = decodeFlowLabel(value);
      continue;
    }
    // Handed straight back on save, so nothing here has to understand either — only that a brace or a comma in one would have ended the value.
    if (key === 'icon' || key === 'img') {
...
// One node, declared with a shape (`A["text"]` or `A@{ shape: … }`) or just named (`A`); a named-only node carries neither shape nor text and the caller supplies both. An opener that leads nowhere is stepped over rather than refused, since several are shared — its brackets are then still in `rest`, no connector matches them, and the statement fails anyway.
function takeFlowNode(rest) {
  const id = FLOW_ID_RE.exec(rest);
  if (!id) return null;
  const after = rest.slice(id[0].length);
  // A class written straight onto the box, which any of the three forms below may carry. Taken here so all three get it from one place.
...
// One connector. Every spelling comes from FLOW_EDGE_TOKENS and every labeled spelling from FLOW_EDGE_LABELED, both generated from the two line/end tables — so there is no list here to fall behind them. Labeled forms go first: they are longer, and their own regexes refuse to match an unlabeled connector.
function takeFlowLink(rest) {
  // `A e1@--> B` names the line, so something further down can point at it — the only thing that does is an animation. The name sits between the box and the connector, and nothing else may: if what follows is not a connector, this is not a line at all.
  let name = null;
...
function takeFlowConnector(rest) {
  for (const form of FLOW_EDGE_LABELED) {
    ...
    if (!match) continue;
    // A "label" of nothing but dots is a longer dotted edge — `-.....->` is a rank hint, not `-.` around the text `...`. Keep looking.
...
// A connector's label, quoted or not. `false` means refuse the diagram; null means there was no label worth keeping.
...
// One body line: a node on its own, or a chain of node groups joined by connectors. `A --> B --> C` is read as the two edges it means, and `A & B --> C & D` as the four. Anything left over at the end — a second statement after a `;`, a trailing `:::class` — fails the whole line.
...
// What beats the parser on a line, named. The first row that matches wins, so the narrower spellings go above the looser ones — `@{` before an edge id, both before a bare hyphen. Nothing here decides anything: a probe only runs on a line the parser has already refused, and the fallback below is the honest answer when none of them fits.
const FLOW_REFUSALS = [
  ...
  // A dash past what the grammar spells: `-->` is two, `---` is three, and one more of either is mermaid's rank hint. Same again for the thick line.
...
// The sentence for a line the parser stopped on. `what` is passed in where the caller already knows the cause; otherwise the probes pick it out.
...
// An odd number of quotes: the label opened and never closed, so the statement carries on to the next line. Counted rather than matched, because a regex for "an odd number of these" is a regex nobody can read.
function flowQuoteLeftOpen(text) {
  return (String(text).split('"').length - 1) % 2 === 1;
}

// Whether what is inside the braces is an animation and nothing else. A single unknown key sends the line back to the box parser, which is where `shape` and `label` are read — and if it is neither, the diagram is refused whole.
...
// What `subgraph …` names: an id and a title, an id alone, or a title alone. Mermaid takes all three and the id is what an arrow can point at, so a group named only by its title uses that title as its id — which is what mermaid does with it too.
...
// Text in, graph out, or null for a diagram we do not model. Everything above the header — YAML front matter, `%%{init}%%`, comments, blank lines — is kept as it was written and handed straight back by renderFlow.
function parseFlow(text) {
  return walkFlow(text).graph;
}

// Why it refused, in one sentence, or '' for text that parses. A second walk rather than a field on the graph: a refusal has no graph to carry it, and the notice needs the sentence only when there is nothing else to show.
...
function walkFlow(text) {
  ...
  if (!header) {
    // Most of these are another kind of diagram entirely, and its own first word says which — the reader's word, not a name we made up for it.
  ...
  const graph = {
    ...
    // Comments and accessibility lines from inside the body. They go back at the top of it rather than exactly where they were: nothing structural holds them in place, and a canvas edit reorders the statements around them.
    notes: [],
    // `classDef` lines, kept exactly as written: they name colors, which is a vocabulary of one word we do not speak.
    classDefs: [],
    linkStyleDefault: null,
    // `click A call fn()` lines, kept whole and doing nothing. The page renders at mermaid's strict level and has no `unsafe-eval`, so they never ran; they are written back so a save does not delete a line the reader wrote.
    deadClicks: [],
    // Every group, flat and in the order they were opened; `parent` is what nests them. Flat because a box names its group by id, and a tree would make that lookup a walk.
    groups: [],
    nodes: [],
    edges: [],
  };
  ...
  // A class, a style or a link style may be written before the thing it paints, and a link style counts the edges that exist once the whole block is read. So they are collected here and attached at the end.
  const painting = [];
  for (; at < lines.length; at += 1) {
    // A label may be written across two lines, and mermaid wraps it where the break is — so the break is part of the label and the statement is not over until the quote closes. Joined here rather than in the statement parser, which reads one line and should carry on doing so.
    const began = at;
    let line = lines[at];
    ...
    if (opened) {
      ...
      // A box may already exist under this name because an arrow above pointed at the group before it was opened — §19 does exactly that. That one is taken back out at the end; a second group of the same name is not.
    ...
    if (turned) {
      ...
      // At the top level the header already said which way it runs, and mermaid ignores a second answer. Refusing beats keeping a line that does nothing.
    ...
    // `e1@{ animate: true }` gives a named line its animation. The same spelling with a `shape` or a `label` in it is a box, so the keys decide which, and only these two keys are ours to write back. `click A "…"` sends a box somewhere. It may be written above the box it names, so it waits with the classes and the styles and is attached once every box is known. A `call` line is kept whole and left dead: a document must not be able to name a function inside the app and have it run.
    ...
    for (const found of statement.declared) {
      ...
      if (!node) {
        // A node mentioned only in an edge shows its own id, which is what mermaid draws. Writing it back as a declaration says the same thing.
        node = {
          id: found.id,
        ...
        // Named in passing outside, spelled out inside: the box belongs where it was spelled out. Mermaid reads it that way and so does the reader.
        node.group = inside();
      }
      if (found.className && !node.classes.includes(found.className)) node.classes.push(found.className);
      if (found.typed) {
        // The typed form may follow a box already drawn in brackets and change its shape — that is what mermaid does with it, and section 14 of the guide teaches it. Two typed shapes for one box is still a guess.
        if (found.shape) {
        ...
        // Two shapes for one node is a document whose meaning depends on which one mermaid keeps. Not one to guess at.
  ...
  // An arrow may name a group, and §19 points one at a group declared further down — so a box invented for that name is only known to be a group once the whole block has been read. Take it back out; the edge keeps the name.
  ...
  // Now that every box and every line is known, what paints them can be put where it belongs. A rule for something that isn't there refuses the diagram: dropping it would be an edit nobody asked for, and keeping it would write a rule that paints nothing.
...
// One box, written the shortest way it can be said: brackets where the shape has them, the typed form where it does not. A shape with brackets is never written typed, so there is one spelling of each box and the round trip has nothing to choose between.
function flowNodeText(node) {
  const shape = flowShape(node.shape);
  const label = '"' + encodeFlowLabel(node.text) + '"';
  // A picture or an icon is only sayable in the typed form, so a box carrying one is written that way whatever shape it has.
  const extra = [];
...
// The `click` line for a box that has one. Written under the boxes because it names one, and in the short spelling: `href` is optional and both forms draw the same anchor.
function flowClickText(node) {
  const tip = node.hrefTip ? ' "' + node.hrefTip + '"' : '';
  return 'click ' + node.id + ' "' + node.href + '"' + tip;
}

// One group and everything inside it, then the groups inside that, one indent deeper each time. Written where the group was opened, so nesting survives.
...
// Graph out, text in. Every node is written as a declaration and every label is quoted: both are always legal, and between them they remove a class of bug and make renderFlow(parseFlow(text)) an identity for anything we wrote.
function renderFlow(graph) {
  ...
  // Boxes in no group first, then the groups, each with its own inside it. A box says which group it is in, so nothing here depends on where it sits in the list — dragging one among its neighbors cannot move it out of a group.
  for (const node of graph.nodes) {
  ...
  // One `class` line per class, naming every box that carries it — rather than `:::` on the box, which the typed form cannot take. Boxes in declaration order, classes in the order they were first used.
  ...
  // By name, not by whichever box happened to wear it first: a group moves its boxes down the file, and the order of these lines must not follow that.
  ...
  // Counted afresh: a link style names an edge by its place in the file, so moving or deleting a line moves what its color is attached to.
...
// All in place: the sheet owns one graph for as long as it is open, and writes it out once.
...
// A group's id is written into the file and an arrow can point at it, so it cannot be the name of a box or of another group. Named after the boxes it was made from is what a reader would expect, but those names are the reader's; `g1` is ours, the way `n1` and `e1` are.
...
// A new group around the boxes named, inside whatever group they were already in. Boxes from two different groups cannot be gathered into one: the answer would have to be which group the new one goes in, and there is no answer.
...
// Take the group away and leave everything it held. Its boxes go back to whatever held it, so a nested group does not fall out of its parent, and nothing it drew is left pointing at a name that is gone.
...
// Connect two nodes, unless that edge is already drawn. Returns the edge either way, so the canvas can select it.
...
// Put a node into a connection that already exists: `A --> B` becomes `A --> X --> B`. The first half keeps the original line, its ends and its label; the second half matches its look and carries no label of its own.
...
// Pull a node out of the middle of a chain and close the gap behind it: what reached it is joined to what left it, then its own lines go. Without the healing, taking one step out of `A --> B --> C` would leave A and C strangers.
...
// Move a node's declaration before another's. Declaration order is what decides where a node sits among the others on its rank, which is the whole of what dragging one does — see the layout.
