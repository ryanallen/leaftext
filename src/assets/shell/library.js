...
// Read the current folder again, keeping where you are. The host calls this after it changes what is in a folder — a paste, a delete, a rename — so the pane shows the result of what you just did instead of waiting on the watcher. No direction: you are standing where you were, and a folder re-read that slid would say you had gone somewhere.
...
function vaultGitItems(vault) {
  ...
  if (repo.atRoot) {
    ...
    if (repo.nested && repo.nested.length) {
      // Their own histories, inside this one. Naming them is the whole of what a reader needs before the warning below says which of them nothing is holding back.
...
window.leafSetVaultStatus = (id, repo, generation = 0) => {
  ...
  const walked = previous && previous.repo && previous.repo.nested ? previous.repo : null;
  const merged = walked
    ? Object.assign({}, repo, {
        nested: walked.nested,
        tracked: walked.tracked,
        unhandled: walked.unhandled,
      })
    : repo;
