...
// Monaco brings the popup, the hover card and the squiggle machinery; the answers come from the host over one round trip — the same corpus the graph and search read, so the popup can only know what the user pointed at. All of it is trigger-driven ([[, #, hover, a pause after typing): quickSuggestions stays off, because a popup guessing prose words is noise, not help.
...
// The Monaco namespace once an editor exists; providers and markers need it outside setup's scope.
let codeIntelMonaco = null;
// Registered once per page load: Monaco keeps providers globally, not per editor, so re-entering the code view must not stack duplicates.
...
// ---- the round trip ---------------------------------------------------------
// Each ask carries a token; the host answers window.leafCodeIntelAnswer with the same token. A keystroke must never wait on the bridge, so an answer that misses its window resolves to null and the popup simply shows nothing.
...
// ---- the toolbar toggle -----------------------------------------------------
// Shares the recess with the padlock, and arrives and leaves with the code view the way the speed reader does with the reading view. Plain words on the tooltip: the person reading a vault has not necessarily met an editor that types back.
...
if (codeIntelButton) {
  codeIntelButton.addEventListener('click', () => {
    ...
    // Take effect now, not at the next keystroke: markers appear or go with the switch, which is what makes the click feel like it did something.
...
// ---- wiring an editor -------------------------------------------------------
// Called by createMonacoEditor once the editor is on screen, and undone by disposeMonacoEditor. The providers register once; per-editor state is only the lint subscription.
...
function setupCodeIntel(monaco) {
  ...
  // Markdown and XML carry links the host can check; the data formats are values, not prose, and the host answers them with no links by design.
...
// The `[[name]]` (or `[[name#heading]]`, `[[name|alias]]`) covering a 1-based column, and the note name inside it. Null when the column is outside every one.
...
// What typing this suggestion should leave behind after the cursor: nothing when the closing pair is already there, the closer otherwise.
...
function anchorSuggestions(monaco, model, position, query) {
  // The anchors on offer must be the anchors of the text on screen, so any keystroke still inside the debounce reaches the host first.
  flushSourceUpdate();
...
// A pause after typing, then one pass: the host walks the buffer's links — the same scan the graph draws from — and answers with the ranges pointing at nothing. Monaco draws the squiggles.
...
function runCodeLint() {
  ...
  // The host checks its copy of the buffer, so the buffer has to be current before the ask crosses — both ride the same channel, in order.
  flushSourceUpdate();
  codeLintSerial += 1;
