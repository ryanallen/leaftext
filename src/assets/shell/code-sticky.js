...
// The Markdown heading you are reading under, pinned at the top of the source. A deeper heading stacks below it; one at the same level or above replaces it, so the rows read as the trail down to where you are.
//
// Ours rather than Monaco's: its stickyScroll contribution is not in the vendored bundle (it wants a symbol provider and a language service we do not ship), and drawing the rows ourselves is what lets them sit ABOVE the page's edge fade rather than under its wash. Monaco's own colorizer draws the line, so a row is the line it stands for.
...
// The positioned box (carries the dissolve below it) and the clipped rows inside it — the rows slide up out of the box as a section ends, so the box clips.
let stickyHost = null;
let stickyRowsHost = null;
// Every heading's section: the heading's own line, the last line it owns, and its level. Document order, so the nesting chain is a forward walk.
...
// Every ATX heading in the buffer, with the span of lines it owns: up to the next heading at its level or above. Fenced code and front matter are skipped — a `#` in either is not a heading.
function markdownHeadingSections(model) {
  ...
  // Close each section at the first later heading that is its equal or its senior; a stack does it in one pass, whatever the nesting.
...
//
// Each row owns a slot: row 0 the first line-height under the top edge, row 1 the next. A section is in the stack when its heading has scrolled past its slot and its last line has not. When that last line is *inside* the slot the section is on its way out, so the stack slides up by how far it has come — which is what makes the swap read as one heading pushing the other off rather than a blink.
function stickyRowsNow() {
  ...
  for (const section of stickySections) {
    ...
    // Its heading is still on screen below the stack, so nothing deeper can be sticky either.
...
function renderStickyHeadings() {
  ...
  // Geometry is Monaco's, read back rather than restated, so the rows land on the same columns the real lines do — gutter width included, which grows with the line count.
...
// Clicking a row takes you to its heading, landing it exactly where the row was standing — so the thing you clicked does not move.
...
// A theme swap rewrites what the colorizer's classes mean, so the rows have to be colored again — the row set has not changed, so nothing else would redraw them.
...
// Called by createMonacoEditor once the editor is up, and undone by disposeMonacoEditor. Markdown only: the other formats have no headings, and their blocks are values, not a trail you can be lost in.
function setupStickyHeadings(monaco) {
  ...
  // A sibling of the editor, not a child: the rows have to out-paint the page's edge fade, which is outside the editor entirely.
