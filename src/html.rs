...
///
/// Phase 1 of [the plan](../../../docs/features/storage/html-site-styles.md) allows exactly what a page carries inside itself: its own CSS, and pictures and fonts written into the file as `data:`. Every `http` and `https` address is refused, so a saved page cannot phone anywhere it was saved from. `base-uri` is deliberately absent: the author's `<base>` is dropped by the sanitizer and the one below is ours, so a directive here would only break our own. Where the folder is not this machine's — a browser — the page reaches what it carries inside itself and the origin the document was fetched from, which is where its own neighbors are. On the desktop the shell's origin is opaque, so `'self'` there names nothing and the policy is the tighter of the two by itself.
