...
//!
//! TEI goes to [`crate::tei`], which knows its conventions. Everything else — sitemaps, feeds, POMs, plists, config files — lands here, where there is no schema to lean on, so the renderer reads the shape of the tree: leaves become labeled fields, repeated flat records become tables, elements holding elements become sections, mixed content becomes prose. Blocks emitted from a single element carry its byte range, so editing anchors as TEI's does.

use crate::*;
use roxmltree::{Document, Node};

/// Child element names that can title their parent section, in priority order. Shared with [`crate::data`], where they are JSON/YAML keys instead.
pub(crate) const LABEL_TAGS: [&str; 5] = ["title", "name", "head", "label", "heading"];

/// Of those, the ones that read as the section's own name and so head it bare. The rest are qualified by their tag: `<author><name>` heads "Author: Ada".
...
/// Nesting past this depth stops recursing and renders the remaining text, so a pathologically deep file can't blow the stack.
pub(crate) const MAX_DEPTH: usize = 24;

/// Parse XML for reading. Doctypes are allowed through (plists and XHTML carry one); roxmltree reads only the internal subset and never fetches anything.
...
/// Whether this document is TEI, and so belongs to the TEI renderer: a `<TEI>` root (any version) or a `<teiHeader>` anywhere in it.
...
/// The block source map for any XML string, matching what [`render_xml_document`] stamps inline. The document `<h1>` is not a block, so no fallback is needed.
pub(crate) fn xml_block_source_map(xml: &str) -> Vec<BlockSpan> {
    render_xml_document(xml, None).2
}

/// A file stem read as a document title: `sitemap` → `Sitemap`, `site-map` → `Site map`. Used when the XML itself names no title.
...
pub(crate) struct XmlCtx {
    ...
    /// Source-anchored editing map: one entry per block that came from exactly one element, in document order.
    pub(crate) blocks: Vec<BlockSpan>,
    next_block_id: usize,
    seen: HashMap<String, usize>,
    /// The element whose text became the document `<h1>`, so it isn't repeated as a field or a section heading further down.
...
impl XmlCtx {
    ...
    /// Record a `kind` block over `start..end` and return the `data-*` attribute string for its opening tag.
...
fn render_generic_document<'a>(
    ...
    // The document's own title, if it has one: a title-ish child of the root, or of the root's single wrapper child (an RSS `<channel>`, say).
    ...
    // Head the document with its title, or with the caller's fallback so the reading view always opens on a heading.
    let heading = title
        .clone()
...
/// The element whose text titles the document, searched in the root and — when the root holds a single wrapper element — inside that wrapper too.
...
/// Render one element that holds other elements (or is deeper than [`MAX_DEPTH`]): a heading, its attributes, then its children.
fn render_element<'a>(node: Node<'a, 'a>, ctx: &mut XmlCtx, depth: usize) {
    if depth >= MAX_DEPTH || has_own_text(node) {
        // Too deep to keep sectioning, or mixed text-and-markup content: render what it says as a paragraph and stop.
...
/// The heading for a container: its labeling child's text, a naming attribute, or the humanized tag name. Returns the source element the label came from so the heading can be anchored to it.
fn container_label<'a>(node: Node<'a, 'a>, ctx: &XmlCtx) -> (Option<String>, Option<Node<'a, 'a>>) {
    // The wrapper that gave the document its title (an RSS `<channel>`) is already spoken for by the `<h1>`; heading it again just repeats.
...
/// Render leaf siblings as one label/value list. Empty elements and the element already used as the document title are skipped.
...
/// Whether `siblings[start..]` opens a run of repeated records worth rendering as a table, and if so where the run ends and what its columns are. A record qualifies when it is a flat bag of short values — same tag, no prose, no grandchildren.
fn table_group<'a>(
    siblings: &[Node<'a, 'a>],
    start: usize,
...
/// Whether an element has text of its own (not counting whitespace between child elements) — the mark of prose rather than structure.
...
/// The reader-facing label for an element or attribute name. A handful of names from the formats people actually open (sitemaps, feeds) read badly when merely humanized, so they get spelled out. Shared with [`crate::data`], so a sitemap and the JSON next to it label the same field the same way.
...
/// Turn an element or attribute name into a sentence-case label: `lastBuildDate` → `Last build date`, `pub_date` → `Pub date`. Names that are already all upper case (`URL`, `ISBN`) are left alone.
