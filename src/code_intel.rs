...
//!
//! Monaco supplies the popup, the hover card and the underline; this module supplies the answers, from what Leaf already holds — the vault's corpus, the document's own folder, and the same link scan the graph draws from. Nothing here reads beyond what the user pointed at: the active vault, or the open document's folder, one level and no further.
//!
...
pub struct NoteItem {
    ...
    pub label: String,
    /// Where it lives, shown small beside the name — the folder, relative to the vault when there is one, so two notes sharing a name stay tellable.
    pub detail: String,
}
...
/// One broken link, as the range Monaco underlines. Lines and columns are 1-based, columns in UTF-16 units — the editor's own coordinates, computed here so the page never re-walks the text.
#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
...
/// The notes a vault can offer, named every way `[[wiki]]` names resolve — a row per file name, and a row per alias, because a name that resolves and is not offered is one somebody has to already know about.
...
/// The notes one folder can offer — what a document outside every vault gets. The names come off the listing; the aliases cost the top of each file.
...
/// One row for the file name, then one per alias. An alias's row says which file it belongs to, because the alias alone does not say what will open.
...
/// The folder shown beside a note's name: relative to the vault root when the note is under one, the folder's own name otherwise, empty at the top.
...
/// Every name `[[wiki]]` links can resolve against, and which notes claim each one. Keys are `normalize_name_key`'s — the same normalization the graph uses.
///
/// More than one claimant is a real state of a vault rather than a mistake in a link: a note preferring a name another note is called, or two notes preferring the same one. The link resolves to the first, and the check can say so.
...
impl NoteNames {
    /// Build from each note's names — its label, then its aliases. Every label goes in before any alias, so a name somebody typed on disk is always the first claimant of it.
    ...
    /// The notes claiming this key, in the order a link resolves them: the first is the one it opens.
...
/// How much of a file is opened to find its aliases. The frontmatter block is at the top of the file, and this is past any real one — a block longer than this has no closing fence inside it, so the note simply has no aliases.
const ALIAS_HEAD_BYTES: usize = 8 * 1024;

/// One note in a folder: the name off the listing, and the other names it answers to. The aliases are inside the file, so this is the only thing here that opens one, and it opens only the head.
struct FolderNote {
    path: PathBuf,
    label: String,
    aliases: Vec<String>,
...
/// Every note one folder offers, with its aliases. One directory listing and a head read per file, capped at [`crate::doc_graph::MAX_DOCUMENTS`] — the cap the map around a loose document already reads to. One folder, never a tree.
...
/// One file's aliases, from the top of it. Unreadable is no aliases, the way an unreadable file is no document everywhere else.
...
/// The same from one folder, for a document no vault holds: the listing's names, and each file's aliases.
...
/// Find the note a `[[name]]` points at among `documents` — the corpus, or a folder's freshly read documents. First match wins, as in the graph.
pub fn find_note<'a>(name: &str, documents: &'a [CorpusDocument]) -> Option<&'a CorpusDocument> {
    let key = normalize_name_key(name);
    // The label pass runs to the end before any alias is tried, so a real file name beats another note's alias — the same order the graph builds its index.
    documents
        .iter()
...
/// Read the note `[[name]]` points at out of one folder, for a document no vault holds. One directory listing, then at most one whole file read.
///
/// A file name is tried against the listing alone, so the common case still opens nothing it does not return. Only a name no file is called falls back to the aliases, which are inside the files.
...
/// A note's opening lines, for the hover card. Markdown as written — the card renders it — cut at a line boundary with an ellipsis when there is more.
...
/// Every link in `text` that points at nothing: a path with no file behind it, or a `[[name]]` no known note answers to. `known_names` is the vault's (or the folder's) note names in `normalize_name_key` form.
///
/// Web addresses are never checked — nothing local can vouch for them — and a link the scan cannot place in the source draws no marker.
pub fn lint_links(text: &str, path: &Path, known_names: &NoteNames) -> Vec<LintMarker> {
    ...
    // One walk of the text turns every byte offset into the editor's own coordinates. Sorted and deduped first: two scans can place overlapping spans, so the flattened ends are not in order on their own.
...
/// What to say about a name more than one note answers to: which note the link opens, and which others wanted it. Nothing when only one note claims it.
...
/// The (line, column) for each byte offset, both 1-based, columns in UTF-16 units — the editor's own coordinates. `offsets` must be sorted; an offset past the end lands on the last position, and one inside a code point on the position of that code point.
