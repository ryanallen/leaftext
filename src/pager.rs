...
/// Turn an on-disk name into a display label (matches the web `label()`): drop the extension of a name the pager lists, collapse `-`/`_` runs to spaces, title-case each word. e.g. `book-1-lower_weir--readings` -> `Book 1 Lower Weir Readings`. `is_listed_document_path` is the gate, the same one that decides which names the pager walks, so a source file or an unreadable ending keeps it in the label rather than being told apart twice.
