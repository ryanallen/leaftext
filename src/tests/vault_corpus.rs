...
fn search_over_a_full_vault_is_timed_not_guessed() {
    ...
    // The best of three, not one run: this machine's spread between runs of the same code is wider than any of the changes being measured, and the first pass pays for faulting 24 MB in. The fastest pass is the one where nothing else got in the way.
...
fn one_read_graphs_the_vault_with_no_database_involved() {
    ...
    // Never opened, never linked: still a node, which is the whole reason to read the vault rather than accumulate as you click.
    ...
    // Two edges: opening–refuge (both directions collapse to one) and the wiki link opening–vows. The dangling link contributes nothing.
    ...
    // Focus keeps the seed and its neighbors; a cap keeps the densest and says the picture is partial.
...
fn the_same_read_answers_search() {
    ...
    // The body hit points at the line it matched and the heading above it, so clicking the result lands in the right section.
    let opening = &hits.hits[1];
    assert_eq!(opening.anchor.as_deref(), Some("opening"));
    assert_eq!(opening.start_line, 3);
    // The snippet marks the match with the control characters the page turns into <mark>, and never carries a newline.
...
fn a_snippet_is_cut_from_the_real_text_not_a_lowercased_copy() {
    ...
    // Case still does not matter, on either path: an ASCII query over text that is not ASCII, and a query that is not ASCII itself.
...
fn a_capped_result_set_says_it_was_capped() {
    ...
    // Every document carries the term, so the list is cut and has to say so. The cap is on files, not rows: fifty files, up to three rows each.
...
fn a_wiki_link_reaches_a_note_by_its_alias() {
    ...
    // Inline form, and an alias that is the file's own name: neither is a second key, and the self-alias must not draw an edge from a note to itself.
    write(
        &root.join("Kv 626.md"),
    ...
    // One edge from both wiki links in listening.md, and none from the note that links to its own alias.
    let graph = corpus.graph(&GraphRequest::default());
...
fn search_finds_a_note_by_its_alias_and_scores_it_like_a_name() {
    ...
    // The whole scale, not a discount: an alias the term matches end to end is worth the top of it, the same 400 a file name matched end to end is worth. Anything less and a note would sort below worse matches for using the field.
