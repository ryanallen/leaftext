...
#[test]
fn a_folder_read_draws_one_list_and_never_two() {
    let html = app_shell_page();
    let css = reading_mode_css();

    // The rows go straight into the tree: no stage, no layer, no copy of the folder being left. One list, at full strength, in the frame the read is answered.
    assert!(html.contains("libraryTree.innerHTML = html;"));
    assert!(!html.contains("library-tree-stage"));
    assert!(!html.contains("library-tree-layer"));
    assert!(!html.contains("runLibraryTreeSwap"));
    assert!(!html.contains("libraryTreeLeaving"));
    assert!(!html.contains("LIBRARY_SWAP_FALLBACK_MS"));
    assert!(!html.contains("librarySwapTimer"));
    // Nothing binds a row twice and nothing waits after the binding.
    assert!(html.contains(
        "bindLibraryRows();
  return true;"
    ));

    // No folder gesture carries a direction any more, and the shared word is gone with both its helpers — a page that animates nothing has nothing to spend.
    assert!(html.contains("function setLibraryFolder(path) {"));
    assert!(html
        .contains("bindLibraryRowPress(button, () => setLibraryFolder(button.dataset.navInto));"));
    assert!(html.contains("setLibraryFolder(crumb.dataset.crumbPath)"));
    assert!(html.contains("run: () => setLibraryFolder(segment.path),"));
    assert!(html.contains("case 'openFolder': setLibraryFolder(path); break;"));
    assert!(html.contains(
  ...
  setLibraryFolder(path);"
    ...
    // A read describing what is already drawn still leaves the rows standing, so an unchanged watcher tick cannot take a row out from under a press.
    assert!(html.contains("if (html === libraryTreeHtml) return false;"));
    assert!(html.contains(
  ...
  send({ command: 'getFolder', path: libraryProjectPath });"
    ...
    // And the stylesheet carries no travel, no keyframe and no going-state for the list.
    assert!(!css.contains("library-tree-stage"));
    assert!(!css.contains("library-tree-layer"));
    assert!(!css.contains("--library-tree-travel"));
    assert!(!css.contains("leaf-library-tree-arrive"));
    assert!(!css.contains("leaf-library-tree-leave"));
}
