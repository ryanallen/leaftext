...
/// Push a terminal update state to the page. Progress is reported by the page itself, so the host only ever sends `staged` or `failed`.
...
/// Fetch and stage an installer, then report how it went. Runs on its own thread: this is seconds to minutes of network I/O, and the event loop has a window to keep painting. The page cannot fetch it — `updater.rs` says why.
...
/// The download proper, returning the version that was staged. A partial file is discarded on the way out, so a failure never leaves bytes that a later launch could mistake for a finished download.
pub(crate) fn stage_update_download(
    ...
    // Repaint on whole-percent changes only: the transfer hands back dozens of chunks per percent, and each one crosses to the web view.
    let mut painted = 0;
...
///
/// If the staged version is the version now running, the install worked and the record is stale — clear it. Then delete every staged folder except one still genuinely pending, which also cleans up the helper copy the last update left behind. Returns true when settings changed and need saving.
pub(crate) fn reconcile_staged_update(settings: &mut Settings) -> bool {
    ...
    // A record pointing at a folder that is gone is worse than no record: the button would offer a restart that cannot happen.
    if !settings.update_staged_version.is_empty()
...
/// Whether a launch should install the staged update by itself. Split out from the work so the one-attempt guard is testable.
...
/// Install a staged update before opening a window: what makes updating automatic rather than merely offered. Returns true when the applier was launched and this process must exit without building any UI.
///
/// The attempt is recorded *before* the helper starts, so an installer that fails silently is tried exactly once and then left to the button in Settings.
