//! Searching the active vault's text: one worker thread, and the counter that
//! tells it a query has been overtaken.
//!
//! A keystroke is a query, and a fast typist sends one every 150 ms. A thread each
//! is eight characters running eight full scans at once and seven answers thrown
//! away on arrival, so there is one thread and one counter: the scan reads it
//! between documents and stops as soon as the field has moved on.
...
pub(crate) struct VaultSearch {
    ...
    /// The last answer, and what it was an answer to. The pane re-runs its search
    /// every time the folder on screen changes, and the field losing and regaining
    /// focus asks again too — neither is a reason to read the vault twice.
...
/// Which query is being waited for. The loop claims a number per keystroke; the
/// scan reads it between documents, and anything holding an older one is work
/// nobody will read.
...
struct SearchJob {
    ...
    generation: u64,
    /// The paths a shorter version of this query matched, when there is one to
    /// narrow to — see [`VaultSearch::narrowing`].
    within: Option<Arc<Vec<String>>>,
}

impl VaultSearch {
    /// Abandon whatever is running. Called when the vault moves: the answer would
    /// be about somewhere we have left.
    pub(crate) fn cancel(&self) {
        self.generation.cancel();
    }

    /// The answer to this query, if it is the one already given and the vault's
    ...
    /// The paths to scan for `query`, when a shorter query has already been
    /// answered over the same text. Typing one more letter can only ever shrink the
    /// set — every term is required, and the longer query's terms contain the
    /// shorter one's — so the documents that missed before cannot match now.
    pub(crate) fn narrowing(&self, query: &str, corpus: u64) -> Option<Arc<Vec<String>>> {
        let last = self.last.as_ref()?;
        // Same text, and this query is the last one with more typed on the end.
        // Anything else — a letter deleted, a different case, a space added — is a
        // different question and gets the whole vault.
...
/// Search the vault's text. Same wait-for-the-read shape as the graph, and the
/// same one read behind both.
pub(crate) fn request_vault_search(
    state: &mut VaultState,
    proxy: &EventLoopProxy<UserEvent>,
    query: String,
) {
    // Already answered, over text that has not changed since. Sent as an event
    // rather than painted here, so a kept answer and a fresh one arrive by one path.
    ...
    match state.corpus.clone() {
        ...
        None => {
            // One slot, so this is last-one-wins already: the counter is claimed
            // when the read lands and the parked query finally runs.
...
pub(crate) fn run_search(
    ...
    // The worker is gone — a scan panicked. Answer this query on its own thread
    // and let the next one start a fresh worker, rather than leaving the box dead.
...
/// One thread for every query there will ever be. It holds the counter rather
/// than the loop's state, because that is the only thing a worker may read while
/// the loop is running.
...
/// The whole scan, run to completion off the worker: the fallback path when the
/// worker thread has died and this query would otherwise go unanswered.
...
/// Paint a finished search, unless the field or the vault moved past it. The page
/// also drops answers to queries it has moved on from, so a slow one is harmless
/// twice over.
