...
//! the new arm instead of leaving it silently reading as Markdown.
...
impl DocumentFormat {
    /// Every format, in the order the file dialog lists them. Callers derive
    /// their lists from this rather than restating one.
    pub const ALL: [Self; 5] = [Self::Markdown, Self::Xml, Self::Json, Self::Yaml, Self::Eml];

    /// The extensions that name this format, lowercase and without the dot. The
    /// first is canonical; the rest are accepted spellings.
    pub fn extensions(self) -> &'static [&'static str] {
        match self {
            ...
            Self::Json => &["json"],
            Self::Yaml => &["yaml", "yml"],
            // MHT web archives are the same MIME envelope as mail, so the one
            // reader opens both.
            Self::Eml => &["eml", "mht", "mhtml"],
        }
    }

    /// The format an extension names, or `None` when the app can't read it.
    /// Case-insensitive: extensions arrive from the filesystem as typed.
    ...
    /// The format `path` names, or `None` when the app can't read it. This is the
    /// question to ask before opening a file; [`Self::from_path`] is the one to
    /// ask once it is already open.
    ...
    pub fn language_token(self) -> &'static str {
        match self {
            ...
            Self::Json => "json",
            Self::Yaml => "yaml",
            // No email grammar is bundled; the code view falls back to plain
            // text, which still edits and minimaps.
            Self::Eml => "email",
        }
    }

    /// The label shown on the code view, the file dialog's per-format filter,
    /// and the fallback highlight class.
...
/// Every readable extension, in format order. The file dialog's combined filter
/// and anything else that needs the flat list.
