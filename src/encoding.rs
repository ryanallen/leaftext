...
//!
//! An encoding belongs to the file, not to the text, so it is learned once when
//! the file is read, carried on the open document, and spent again when the file
//! is written. Saving never changes how a file is spelled.
//!
//! Everything here is `std`.
//!
//! A byte order mark is taken off the text and remembered rather than left in it.
//! Left in it is a character at offset zero, so pulldown-cmark reads
//! `\u{feff}- [ ] a` as a paragraph, `---` stops opening frontmatter, and
//! `serde_json` refuses the document. [`encode_source`] puts it back.

use crate::*;

/// How many bytes of a file are searched for the zero byte that says "this is
/// not text". Enough to catch a real binary's header without reading far.
const BINARY_SNIFF_BYTES: usize = 8 * 1024;

/// How a file's bytes spell its text.
///
/// Every one of these is *declared* by the file, by its byte order mark. There is
/// no variant for a legacy code page, because nothing in such a file says it is
/// one: those are read as Windows-1252 and the text is UTF-8 from then on — see
/// [`decode_source`] — so the app never writes a guess back out.
...
/// How a file spells its text: which encoding, and whether it opens with a byte
/// order mark. The two travel together because writing the file back needs both.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub struct SourceSpelling {
    pub encoding: SourceEncoding,
    /// Whether the file began with a mark. Kept as a fact about the file rather
    /// than as a character in the text — see the module comment.
    pub mark: bool,
}
...
/// A document's text together with how the file it came from spells it. The text
/// never carries a leading mark; `spelling.mark` remembers it instead.
...
impl SourceText {
    /// UTF-8 text that came from somewhere other than a file read — the code
    /// view's unsaved buffer, a test fixture.
...
///
/// Check order is load-bearing: four-byte marks before two-byte ones, or UTF-32 LE
/// (`FF FE 00 00`) reads as UTF-16 LE and comes out as garbage rather than an error;
/// and valid UTF-8 before the zero-byte test, since `U+0000` is legal UTF-8.
...
pub fn decode_source(bytes: &[u8]) -> Result<SourceText, String> {
    // A wide mark decodes along with everything else and then comes off the front
    // of the text, so no parser downstream ever meets one.
    ...
    match std::str::from_utf8(bytes) {
        ...
        Err(_) => match first_zero_byte(bytes) {
            ...
            // Some legacy code page, and nothing says which. Windows-1252 always
            // decodes, so the file opens and mojibake is at least visible. The text
            // is UTF-8 from here on: writing the guess back would drop any character
            // it has no room for, and losing text beats re-spelling a file.
            None => Ok(SourceText {
...
/// Write `text` back the way the file was spelled when it was read. The one way a
/// document reaches the disk, so a save can't quietly re-encode one.
pub fn write_source(
...
/// Encode `text` as `spelling` describes, putting back the mark the read took off.
/// The wide encodings always get one: it is the only thing identifying them, so an
/// unmarked UTF-16 file is one this app could not open again.
...
/// `text` without a leading mark. Only the first one: a `U+FEFF` anywhere else is
/// a zero-width no-break space the author put there, and not ours to remove.
fn strip_mark(text: &str) -> String {
    text.strip_prefix(MARK).unwrap_or(text).to_string()
}

/// The encoding a leading byte order mark declares, if there is one. Four-byte
/// marks are tested first: `FF FE 00 00` is UTF-32 LE, and its first two bytes
/// are also the UTF-16 LE mark.
...
/// Offset of the first zero byte in the sniffed prefix — git's test for a file
/// that is not text.
...
fn decode_utf32(bytes: &[u8], big_endian: bool) -> Result<String, String> {
    ...
    bytes
        ...
        .map(|quad| {
            ...
            // `from_u32` already refuses surrogates and anything past 10FFFF,
            // which is the whole of UTF-32's validity rule.
...
/// Decode as Windows-1252, which cannot fail: ASCII below `80`, Latin-1 from `A0`,
/// and the table below between. The five bytes Windows leaves undefined map to their
/// C1 controls, as WHATWG specifies, so all 256 values have an answer.
