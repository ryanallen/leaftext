...
//
// Drawn once, at the publish: `bakeSite` in `scripts/site-assets.mjs` draws the chart into the first response, and `web/preview/boot.js` lifts that drawn chart out of the page it was served rather than drawing it again. The bake has no DOM, so everything here works on the renderer's HTML as text; that HTML is the app's own and regular, which is what makes a text pass safe.
//
