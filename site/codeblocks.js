...
// Load highlight.js once per page, keyed by src (the two pages reach the vendored file by different relative paths).
...
// Give every fenced code block (but not Mermaid diagrams) a "copy all" button, matching the desktop app. The button copies the code verbatim; code.textContent is the raw source whether or not highlight.js has wrapped it in token spans.
...
// Copy via the async clipboard API, falling back to a hidden textarea + execCommand where the async API is unavailable (e.g. non-secure contexts).
