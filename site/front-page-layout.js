...
//
// Two callers, one layout: the publish bakes it into the first response (`scripts/site-assets.mjs`), and `reader.js` lays out a page nobody baked. The bake has no DOM, so everything here works on the renderer's HTML as text, the same way `compare-chart.js` does.
// ---------------------------------------------------------------------------
...
function leadOf(section) {
  ...
  return paragraphs(cut < 0 ? top : top.slice(0, cut)).filter((p) => !/<img\b/.test(p));
}
...
function cardOf(card, section) {
  ...
  return `<article class="front-card" data-clip-card="${card.clip}"><h3 id="${card.section}">${card.title}</h3>${lead.join('')}${clipBox(card.clip, card.shows, 'front-card-clip')}<p class="front-more"><a href="${card.more}">More →</a></p></article>`;
}

/**
 * The README drawn as the front page: hero and downloads, the comparison, the app, the four cards, the install line and the foot.
 *
