...
// produced. Each entry links to its heading's id and the entries nest as an
// ordered list — one step in per step down in heading level (1., then 1. again a
// level deeper) — inside a <details> that starts closed so it never crowds the
// top of the page. Clicking an entry jumps to that heading via its #id.
// ---------------------------------------------------------------------------
...
// Turn a flat, document-order list of headings into a nested <ol>, one level of
...
function buildOutlineList(doc, headings) {
  ...
  headings.forEach((h) => {
    ...
    if (parent.level !== 0) {
      ...
      if (!sub) {
        sub = doc.createElement('ol');
        (lastLi || parent.ol).appendChild(sub);
      }
      container = sub;
    }
