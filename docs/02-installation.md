...
Installing registers Leaftext as a handler for every extension it reads — `.md`, `.markdown`, `.mdown`, `.mdc`, `.html`, `.htm`, `.xml`, `.json`, `.yaml`, `.yml`, `.txt`, `.ini`, `.eml`, `.mht`, `.mhtml`, `.docx`, `.xlsx`, `.pptx`, `.odt`, `.ods`, and `.odp`, along with the [source-file](01-features/01-rendering.md#source-files) extensions — so those files carry the leaf icon and appear under **Open with**. On Windows the entries are per-user (`HKCU`), like the install itself. HTML remains assigned to the browser, plain text to Notepad or TextEdit, `.ini` to whatever opens it today, and Word, Excel, PowerPoint and OpenDocument files to the application that owns them, unless you choose Leaftext.
