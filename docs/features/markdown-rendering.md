...
Every block gets a citable address derived from its place in the document. A heading keeps its text slug, so `#slug` links and the table of contents still resolve (a heading with no slug falls back to `section-<n>`). Every body block after a heading — paragraphs, quotes, list items, tables — takes that heading's slug plus its running position in the section, like `foreword-p-3` for the fourth paragraph under `foreword`, so a paragraph is numbered under the heading it belongs to. Clicking a gutter permalink jumps to the block **and** copies its `#<id>` to the clipboard, so you can paste the citation out — the way to read it on touch, where there is no hover tooltip. A block with an explicit author `id` keeps that id.
