...
A second custom scheme, `leaf-asset://` (`http://leaf-asset.local/` where custom protocols are restricted), served by `bundled_asset_response()` in `assets.rs`, provides the renderer's vendored runtimes — the Mermaid bundle, the KaTeX script and stylesheet, and the KaTeX WOFF2 fonts — straight from bytes compiled into the binary with `include_bytes!`. Diagrams and math therefore render fully offline, with no CDN dependency, and the Content-Security-Policy restricts script, style, and font sources to `'self'` plus this protocol.
