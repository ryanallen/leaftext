...
- **`src/lib.rs`** — Core document rendering and app-state helpers. Contains `render_markdown_document()`, the TEI/XML rendering, glossary auto-linking, the minimap model builder, settings persistence (`load_settings()` / `save_settings()`), the `local_image_protocol_response()` handler for the `leaf-image://` custom URL scheme, and `app_shell_html()`, which assembles the WebView page by loading the shell markup and script from `src/assets/` (see below) and substituting runtime tokens.
