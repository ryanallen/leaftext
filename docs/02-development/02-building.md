...
| Shared skill copies | `just check-learn-snapshots` | Fail on a copy of a skill in the shareable workflow article that no longer matches the skill it was taken from, except a private workspace pair that was already cut across a primary edit. `node scripts/check-learn-snapshots.mjs --fix` rewrites every pair this checkout changed and leaves an inherited mismatch for the primary checkout |
