...
| Docs coverage | `just check-docs`          | Fail on a Markdown file that nothing keeps true, and on a relative `.md` link that opens nothing. `node scripts/check-docs.mjs --list` prints every one and its role |
