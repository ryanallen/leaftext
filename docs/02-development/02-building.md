...
In a private session, `check-learn-snapshots` still fails a pair the session changed. A pair that was already mismatched when the workspace was cut is reported without stopping the suite, and `--fix` leaves it alone for the primary checkout to settle.
