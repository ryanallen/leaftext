...
| Shared skill copies | `just check-learn-snapshots` | Fail on a copy of a skill in the shareable workflow article that no longer matches the skill it was taken from. `node scripts/check-learn-snapshots.mjs --fix` rewrites every copy from its source |
