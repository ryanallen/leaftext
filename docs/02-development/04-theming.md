...
App surface and text colors, named by role: `--lt-background`, `--lt-foreground`, `--lt-surface`, `--lt-surface-elevated`, `--lt-surface-muted`, `--lt-surface-sunken`, `--lt-border`, `--lt-border-strong`, `--lt-muted-foreground`, and the semantic role tokens `--lt-primary`, `--lt-primary-ink`, `--lt-accent`, `--lt-accent-ink`, `--lt-danger`, `--lt-warning`, `--lt-success`, `--lt-done`, `--lt-link`, `--lt-link-hover`, plus `--lt-focus-ring` and the two focus-selection colors.

A role has a `-foreground` partner only where something prints on it: `primary`, `accent`, `danger` and `success` have one; `warning`, `done` and `link` do not. `primary` and `accent` also have `-ink` partners for the same role as words or a thin mark rather than as a fill, because a fill and an ink pull the color opposite ways; see [Fills, foregrounds and inks](#fills-foregrounds-and-inks).
...
Corners are tokenized as a **global scale** in the stylesheet's own `:root` block rather than per-theme values: `--lt-radius-xs/sm/md/lg/xl/2xl/pill/full`. Every surface pulls from it, so rounding swaps in one place. Two tokens sit beside the scale rather than on it, because they are one platform's own figure rather than a step: `--lt-radius-window` is 20px, the corner macOS rounds a window at, and `--lt-radius-window-inner` is 12px, that number less the 8px gutter between the window's edge and the reading card's, so the two arcs share a center and the chrome band between them holds its width the whole way round. A Windows window keeps `--lt-radius-lg` and its card `--lt-radius-md`. The window reads its corner from `--app-window-radius` and the card from `--reader-card-radius`, each declared once on `.app-surface`, so a full-screen or maximized Windows window flattens the window by taking that one variable to zero rather than by writing a corner of its own. Elevation has no scale at all: a floating surface throws the same dot lattice the grain is made of — a halftone shadow, said once in the stylesheet and shared by every menu, panel, sheet and tooltip, and by the Previous / Next card at the foot of a page, which throws it only under the pointer. How heavy it is has two answers rather than one, split by which side of the window the band falls on: every band thrown inside the window reads `--lt-grain-shadow-opacity-inside`, and a light appearance takes that to a fifth, because the strength that reads as depth against a desktop reads as ink printed on paper against a page of near-white. That override is keyed on `data-leaf-appearance` rather than on `data-theme`, since a published page writes a family name into the latter and a reader on a dark page would be caught by the wrong selector. The hole the band punches in itself is cut on the host's border box rather than its padding box — one `--lt-shadow-host-border` further out — so a surface's own outline keeps its own color instead of wearing the innermost dot row; the correction comes off the layer's padding rather than its inset, because the lattice is anchored to the window and an outer edge moved one pixel re-phases every dot row against it, and a host that draws no border declares that variable at zero so the band still meets its face. It is a halftone rather than a fade: four edge ramps lift coverage off a soft per-tile blob before one alpha curve cuts what is left, so each dot narrows toward the band's outer rim while the ink it is painted at never moves, and the same four ramps run again in the mask to take the band to nothing at that rim rather than ending it on the faintest row it still draws. The window itself throws one too, at `--lt-grain-shadow-opacity`'s own full strength whatever the appearance, because that band falls on the desktop rather than on a page of the app's and so is read against everything behind the window: the platform's own smooth shadow is turned off on both systems, the window and the web view are see-through, and the app is held `--lt-shadow-spread` — 12px — off the window's edge on all four sides so the band has room to fall in, which is the depth the dot needs to have more than one width in at the resolution the app is read at. It is zeroed where there is nothing behind the window to cast onto — a maximized Windows window, a Mac in full screen — and in a browser, which has no window of its own. The app has one light, hung overhead and centered, so there is no smooth blur left in `--lt-shadow-*` at all: every one of them is a stroke, a recess or a ring, and a `box-shadow` with a vertical offset would be a second light pointing somewhere else. What lifts a surface off the page is the halftone and nothing besides — never a hover, because pointing at a control changes its color.

### Surface grain

Tinted surfaces are not painted as flat fills. A fine dot grid — a 2px lattice of near-transparent dots, 3px on a display at 100% — is tiled over them, so a surface reads as a dithered cell rather than a wash of color. Like the radius scale, the grain lives in the compiled `:root` block rather than per-theme, as four alpha values, the first three darkening further for dark appearances:
...
The lattice's three lengths are hoisted even though the gradient is not, because a length carries no ink: `--lt-grain-radius`, `--lt-grain-edge` and `--lt-grain-tile` sit in the stylesheet's own metrics block and every one of the rules draws from them, so the geometry is said once rather than copied into two dozen rules. That is what lets one branch answer a whole class of display: the shipped dot is 0.6px across in a 2px tile, drawn in device pixels, so at 100% all four pixels of a tile sit 0.7071px from its center — just outside the dot's 0.7px edge — and not one of them is ever painted, which left a table header, a tinted row, a code block and the outline reading as flat washes. A `@media (resolution <= 1dppx)` branch hands that display the same three lengths multiplied by 1.5, so it draws the device-pixel pattern a 150% display draws. `just check-grain-in-a-picture` photographs the fixture at both scales and refuses a flat surface at either.
...
Anchoring it that way costs the page nothing on screen and everything in a file. The render behind [Export the page](../01-features/02-navigation.md#export-the-page) — the same one that writes the PDF and the picture of the whole document — paints a fixed-attachment background as nothing at all unless the box holding it composites, so every table header, tinted row, code block and outline reached both files as a flat wash while the stylesheet said the right thing the whole time. The paper rules the app puts on to render give `.app-surface` a `will-change: transform` for exactly that, and nothing in the window changes because the class is on for the render and off everywhere else. Containment is not what turns it on: putting `contain: paint` or `contain: layout` back under the paper class, or taking the anchoring away with `background-attachment: scroll`, each wrote a picture identical to the one with the grain missing, and a bare `transform: translateZ(0)` does nothing because the engine flattens it to an identity and the box stays untransformed. `just check-grain-in-a-picture` is what holds it: it photographs a fixture the way the Export button does and refuses a code block that comes back one shade.
...
**A new rule that spends `--lt-markdown-link`, one of the five alert colors, `--lt-primary-ink`, `--lt-accent-ink`, `--lt-warning` or `--lt-danger` as `color:`, `fill:` or `stroke:` owes a row.** A row's surface can be one token or the exact fixed-percentage mix a stylesheet paints, so the sync count is measured on its 16% accent wash rather than on a flat color the app never draws. The list is also what the inactive window is measured through: that state re-points no color at all — it is one `filter` on the app surface — so the gate puts both halves of every row here, and of every readable pair, through the same saturation and contrast the browser applies and reads the ratio after, against a floor of 3:1 for that one state.

### Fills, foregrounds and inks

The action color has two jobs that pull opposite ways. As a **fill** — a filled button, the active tab's mark, a search-hit wash, a border — it has to leave enough contrast for `--lt-primary-foreground` to print on it. As an **ink** — the start screen's list headings, a folded sheet's title, the heart on a favorite, the leaf beside a file name, the app-bar logomark, a document's outline and ghost buttons — it has to read on the page. On Goldenrod's light half the two windows do not overlap at any value, so `--lt-primary` stays the fill, wash and border while `--lt-primary-ink` carries the role as words or a thin mark.

`primary-ink` carries a `Default` of `primary` in `design/colors.md`, so a family that says nothing compiles the fill's own value into its block as a hex and not one pixel moves. Four halves declare a second value today — Goldenrod light and Ginger light take the darker gold and darker orange each already carries as its link, and GitHub dark and Sage light take a step of their own. `the_action_and_accent_colors_paint_words_and_marks_out_of_ink_tokens` refuses a rule that goes back to spending the fill as `color:`.

The state accent follows the same contract: `--lt-accent` remains the fill, wash and structural border, `--lt-accent-foreground` prints on its solid fill, and `--lt-accent-ink` paints state words and thin marks over the surface beneath them. Twelve source halves declare a separate ink because their fill misses at least one real surface; the other ten copy their accent. `the_action_and_accent_colors_paint_words_and_marks_out_of_ink_tokens` refuses either fill being spent as `color:`, the readable-pair gate holds the foreground on its fill, and the paint list holds the ink on each stylesheet surface.
