...
> Bump `Cargo.toml`, leave it in the working tree, then run `just release <version> <message>` to commit, tag, and push. CI automatically builds the Windows MSI and the macOS DMG.

Leaftext releases are managed with a single `just release <version> <message>` command — but it does **not** bump the version for you. You edit `version` in `Cargo.toml` to the new value and leave it uncommitted; `just release` then verifies that `Cargo.toml` already matches the version you pass, writes that version into `Cargo.lock` and reads it back, retires the older tags, and commits the work sitting in the tree, tags it and pushes. **It runs no check suite**: a plan pays for that once, at the end of the build that wrote it, and nothing between that run and this one edits a line of code. CI takes over from here to build all platform artifacts and attach them to the GitHub Release.

**The tree is dirty on purpose.** A release commits the work written in this checkout and not yet committed, so a clean tree is nothing to release and is refused before anything else is read. `just land <message>` is the separate command that puts the tree on `main` immediately with no version and no tag, so work stops sitting uncommitted while the documentation and comment passes run; the release then commits whatever those and the version bump add on top.
...
**`prepare-release.mts`** — A TypeScript script (run directly by Node.js via `--experimental-strip-types`) that guards and finalizes the release, from the version reading to the last push. It reads `Cargo.toml` and throws if the version does not already equal the one you passed, requires the tree to have work in it at all, and requires the tag not to exist yet. It then writes that version into the app's own entry in `Cargo.lock` and reads it back, refusing a lockfile it could not bring to the requested version; reads the Mac half's newest completed answer; asks GitHub which release is published; reads the work in the tree again; deletes every older tag here and on the remote; stages that list of paths by name; creates the release commit and an annotated tag; pushes `main`; and pushes the tag on its own.

**The lockfile is written here and nowhere else.** The version is bumped after the build has run its one check, so nothing before this point brings `Cargo.lock` to the new number — and both release builds pass `--locked`, so a lockfile still naming the version before refuses to update one and dies on its first command, with the tag already on GitHub and no installer under it. That is how v1.15.4 went out. It used to be rewritten by accident, as a side effect of the check suite compiling here, and picked up by reading the dirty tree a second time; now it is written on purpose, read back before any tag is touched, and staged by the one read that follows it.
...
It was two commands, the second of which pushed after the first had exited, so a push could outlive whatever the first command had read. It is one command now. It never edits `Cargo.toml` — the version bump is yours to make, and it is one of the paths the release commits.
...
**The current version and the one before it are kept.** Each platform job deletes every release older than those two, along with its tag, once its own upload succeeds, so the releases page holds two complete versions. The second one is what somebody arriving mid-release downloads: the new release does not exist until its build publishes, so a page holding only the newest has nothing complete on it while a release is under way, and a build that fails never publishes at all. That cleanup runs after publishing and can never fail the build: both jobs race to do it, so whichever finishes second routinely finds a release, or its tag, already gone.
...
**The website publishes on a push to `main` rather than on a tag**, through a fourth workflow that is nothing to do with a release. It builds the renderer, writes it beside the pages under `assets/leaftext/`, encodes every screenshot as lossless WebP and moves the pages onto it, and deploys — so neither a compiled module nor a converted picture ever enters the tree, and the site and the app cannot drift apart. The pictures are the reason it also installs `cwebp`: the masters stay PNG in the repository, the WebP is written beside each one on the runner, and the same pixels reach a reader at 3,318 KB rather than the 5,336 KB the masters weigh. It refuses to deploy a build that produced no working module, and GitHub Pages keeps serving the last successful deployment, so a failure means the site stops updating rather than going dark.
...
That is the whole suite — formatting, `cargo check --all-targets`, the browser build, the tests, and every `check-` recipe the `Justfile` defines, each of them listed in [Building](02-building.md#individual-tasks). It runs once a plan, at the end of the build that wrote it. `just release` does not run it again: the same code was proved there, and nothing between that run and the tag edits a line of it.
