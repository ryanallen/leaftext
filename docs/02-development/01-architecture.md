...
- **`src/editing.rs`** — The [editing](../01-features/07-editing.md) model: `EditableDocument` (the per-tab source buffer with dirty tracking, a save version counter, `replace_range()` splices for inline edits, `toggle_task()` for checkbox flips, and the bounded [undo](../01-features/07-editing.md#undo) stack), `render_source_view_html()` (the code view's color layer, reusing the reader's own highlighter), `block_source_map()` — a `pulldown-cmark` `into_offset_iter()` pass mapping each top-level Markdown block to its exact source byte range — and `task_marker_offsets()` for the interactive checkboxes. These source maps are what [inline editing](../01-features/07-editing.md#inline-editing-the-reading-view) anchors its splices to. Public types re-exported at the crate root.
...
- **`src/assets/shell/`** — the in-page application script, as ordered fragments: the app bar and tabs, the library pane and its search, the PixiJS + d3-force [graph](../01-features/03-library.md#graph) scene, the [code view](../01-features/07-editing.md#code-view) editor surface, the reading-view editor and its DOM-to-Markdown conversion, the minimap and reader scroll anchoring, the glossary sheet, theme and locale wiring, and the updater. They are **one script, not modules**: `APP_SHELL_SCRIPT_PARTS` in `lib.rs` concatenates them in order and they share one scope, because the page has no module loader. Order is therefore load-bearing — the last fragment ends with the bootstrap call that must run after everything else is defined.
...
- **`src/assets/theme-bootstrap.js`** and **`src/assets/locale-bootstrap.js`** — the two scripts injected inline before first paint, so the theme resolves and the interface has text on the first frame. The locale script carries the full English / Simplified Chinese string table.
