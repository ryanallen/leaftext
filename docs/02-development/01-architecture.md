...
- **`src/theme.rs`** — The theme system. The semantic token contract (`LEAF_SEMANTIC_TOKEN_CONTRACT`), the `ThemeSource` / `ThemeFile` types, and the CSS compiler: `theme_sources()` (which parses the bundled `src/assets/themes.json` once and leaks it to `&'static`), `compiled_theme_css()`, `theme_source_token_value()`, `assert_theme_sources_cover_contract()`, and `reading_mode_css()`. Palettes are data, not code — they live in `themes.json`, not Rust tables. Its items are `pub(crate)` and re-exported at the crate root. See [theming](04-theming.md).
...
- **`src/assets/themes.json`** — every theme's [palette as data](04-theming.md#palettes-are-data-themesjson), parsed once at startup. It's compiled from the editable `themes/` folder (per-family files + a manifest) by `scripts/bundle-themes.mjs`; `just check-themes` guards the two against drift.
...
`app_shell_html()` generates the full HTML/CSS/JS shell. `reading_mode_css()` assembles the complete style block: compiled theme CSS (from the bundled `themes.json`) + the `:root` alias layer + application CSS. Fonts are not bundled — the active [theme](../01-features/06-themes.md#fonts) fetches its font from Google Fonts on demand. The rendered document HTML is injected into the shell via `workspace_state_script()` or `workspace_switch_script()`, which call the appropriate `window.leaf*` JavaScript entry points.
