...
| `/workspace` | Where a session's own private copy of the plan folder and the app comes from, how a finished result is handed over on a branch of its own, and how one is applied to the shared copies |

Six hooks back the parts of this that are worth failing rather than remembering — they print the standing rules before each message, write down the steps of the skill a message names, put the session in a private copy of its own, refuse a git write without an explicit release, hold a reply to the repo's own rules about how it is written, and prove those rules were read rather than recalled. `just verify` self-tests all six. Each record they keep belongs to one session, so two people working the same checkout cannot spend each other's release, clear what the other has read, or be held to the other's steps; a machine where no session can be identified is refused every git write rather than let through.

## Two at once

Nothing here is shared between two sessions working at the same time. Before a message that names a skill which changes something is even read, a hook gives that session a private pair of worktrees — one carrying the plan folder beside this repo, one carrying the app inside it, at the same path shape the ordinary checkout has — and tells the agent where both halves are. Nobody types a command for this, and a session only ever makes its own. Two sessions therefore keep their own plan edits, their own app source, their own staged changes and their own build folder, which is what a shared checkout could not do: one build held the other, and a ranking pass reset a status the other had just set.

A finished result leaves that copy as a **handoff**: the app change packaged as a patch against the revision the copy was cut from, written beside the plan changes and committed on a branch of that session's own in the private plan repository, pushed only there. It never tags, never moves the version, and never speaks to the app's own remote. Applying one to the shared copies is a separate step that takes a reservation so only one arrives at a time, checks the recorded revisions against what is actually there, refuses a result overlapping work already sitting in a shared copy, and writes both halves through a recovery journal — so a run that fails or is killed puts both copies back rather than leaving them half-written. It leaves the shared copies dirty on purpose: what arrived is read, and then the public release below is made from there. That release refuses to run from a private copy at all.
