...
**6. Ship it, then close it.** `/git-release` is the only step that writes git, and its first act is to put the work in the tree onto `main` — unchecked, on purpose, so it stops sitting in a checkout while the docs, the comments and the whole suite run. The release commit that follows carries whatever those write. **Only a change somebody running the app can meet gets a version number and a tag** — the skills, the hooks, the checks and the scripts sit in the same checkout as the app and are not the app, so they land and stop. A release workflow holds both kinds at once and is read by which step moved: every step up to and including the one that makes the installers takes a number, and every step after the installers exist — making the release, uploading them, clearing the old ones — takes none. `/done` retires the plan afterwards, on the owner's word alone.
