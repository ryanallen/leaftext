...
A build is watched the same way. The prompt hook writes down which designed ticket a build message named, the edit hook samples that ticket before every source edit — which phase is being built, and how many of its boxes are ticked at that moment — and the reply hook reads the samples at the end of the turn. A turn is held when the phase gained no tick at all, and again when more than one box was ticked after its last source edit — one box past that edit is what the loop may always still owe, because a box's code goes in before its own tick. So a build that writes a whole phase and ticks it all at the end is refused, and so is one that ticks its first box honestly and then sweeps the rest behind it; one that ticks each box as it finishes is not. The difference is the order, and the order is the only thing that tells the owner reading the ticket whether anything is happening.
