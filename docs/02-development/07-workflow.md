...
Where two run anyway, a build leaves the other one's work alone: a ticket this build was not pointed at — half built, fully ticked, mid-round, failing the gate — stays byte for byte where it sits, not retired, not repaired, not filed about, because the session working it is already its record. The rule is still one at a time — a release stages the tree in front of it, so the second session's work rides out with the first's.
