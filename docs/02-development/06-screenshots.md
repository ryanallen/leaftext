...
| `update.png` | The update bell wearing its green dot, and the panel under it holding one Restart to update button | **Run 15 August 2026.** Not a `-Doc` run: the state is staged rather than waited for. Launch a copy of your own against a throwaway profile — the settings this script writes, `fern` light at 1000x799, library shut, any document — then run three lines in it with `just ask '{"ask":"eval",…}'`: `window.leafUpdateState({status: 'staged', version: '1.15.0'})` draws the bell, its dot and the button; `document.getElementById('updateMenu').open = true` drops the panel; `document.querySelector('.reader-shell').scrollTop = 1800` puts blank page under it, because the crop's foot otherwise cuts a heading in half. Then `-Attach -Crop '586,4,480,190'`, and close it with `just ask '{"ask":"quit"}'` rather than a kill |
