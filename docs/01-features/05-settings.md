...
There is nothing to open. Every control changes the app the moment you use it and saves straight away. The version you are running is at the foot of the home screen ([Updates](#updates)), and **Licenses** beside it opens every third-party notice the app carries — the icon packs, the libraries behind math, diagrams, the source editor and the graph, and the licenses of all 176 Rust packages compiled into the binary — as a document in the reader, with no network needed.
...
- **Every launch asks GitHub for the latest release**, unthrottled — opening the app is when you expect it to know whether it is current. A window left open re-checks at most every six hours while active; returning from another app checks if that time has passed. `update_last_checked` records when it last did, so a long session does not spend requests against the rate limit
- When a newer release exists, its installer downloads in the background — the same `.dmg` or `.exe` published for hand-installing, since a release carries nothing else. A download that arrives short or oversized is deleted rather than kept, and the digest of what did land is recorded so the installer can be re-checked before it runs
