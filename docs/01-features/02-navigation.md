...
- Reopening Leaftext puts its saved file tabs back in the same order and opens the tab that was in front, at its last reading or source position; the other documents wait to load until you select them. Files that are no longer there do not return, and neither does a [new document](07-editing.md#new-document) nobody typed into.
