...
| [Sorting and filtering what is drawn](08-relational-tables.md#views-sort-and-filter) | Point at a table and its own bar offers a sort and a filter that change what you are looking at and never the file; the sort on the column handle above is the one that writes |
| [The table's right-click menu](#editing-a-table) | Right-click a cell for the same row and column actions the handles carry, and for how that cell gets its value |
...
- **The same actions on a right-click.** Right-click any cell of an open table and the menu carries every row and column action the handles do, acting on the cell you right-clicked, with [the five that say how that cell gets its value](#a-table-that-adds-itself-up) under them.
