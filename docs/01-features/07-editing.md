...
- Edits raise the same green **Save** button and unsaved-dot as the code view, and save the same way — from your first keystroke, without clicking out of what you are typing in. **Save**, **Undo** and `Ctrl+S` all act on the words on screen, and the typing itself reaches the document at every pause, so an outside change arriving mid-sentence no longer takes it away. Clicking out of a paragraph is what turns the marks you typed into what they mean — `*stars*` into emphasis, an address into a link — and it draws that one paragraph again and leaves the rest of the page standing, so moving from line to line in a long note costs no wait. A block of any other kind, or a paragraph whose words now reach past it — a new blank line, a list marker, a bracket the rest of the note might answer — draws the whole page again.
