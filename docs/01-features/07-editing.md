...
- **A grip beside the row and above the column, and it drags.** Run the pointer over a table and one handle stands at the body row you are level with and one above the column you are in. Take the row grip and carry it down or up: the row lifts and follows the pointer, the rows it passes step aside to open the place it will land, and letting go drops it there. The column grip does the same sideways, and the columns it crosses step aside for it. A release anywhere outside that table cancels, leaving it exactly as it was, so a drag that wanders into the document never moves a row out of the table. The dots mean that one thing and nothing else — a press that never becomes a drag does nothing. The header row has no grip of its own — the pipes are keyed off it.
