...
| [Row and column grips](#editing-a-table) | Point at a table and a grip appears beside the row and above the column: drag either to reorder it |
| [Sorting a column](#editing-a-table) | Sort the body rows by any column, A to Z or Z to A — numbers and ISO dates by value, everything else as text; the header, the divider and a row of totals never move |
| [A date, a choice or a link, picked rather than typed](01-rendering.md#tables) | With editing on, a cell holding a date, one of a short list of values, or a link to a row of another table opens a small picker instead of a caret; whatever you pick rewrites that one cell and leaves the rest of the table's spacing exactly as you set it |
| [Relational tables](08-relational-tables.md) | A table can be viewed as cards, a board or a list, filtered, sorted and linked to a row in another table without leaving Markdown |
...
- **A date on a task line is picked rather than typed.** A checkbox whose line ends in a `📅 09/16/2026` date draws that date as a short badge after the words — `Sep 16`, red once it is past, green on the day itself, amber through the next seven days, gray later or once the box is ticked. Press the badge and the same menu every other picker uses offers today, tomorrow and next Monday, a box for any other date, and a row that takes the date off the line. Whichever you pick rewrites only that date's own characters, so the rest of the line and every other line is left exactly as you wrote it, and one press of undo puts the old date back. It is an ordinary edit, so it waits for Save. See [Task lists](01-rendering.md#task-lists).
...
- Edits raise the same green **Save** button and unsaved-dot as the code view, and save the same way — from your first keystroke, without clicking out of what you are typing in. **Save**, **Undo** and `Ctrl+S` all act on the words on screen, and the typing itself reaches the document at every pause, so an outside change arriving mid-sentence no longer takes it away. Clicking out of a paragraph or heading is what turns the marks you typed into what they mean — `*stars*` into emphasis, an address into a link — and it draws that one block again and leaves the rest of the page standing, so moving from line to line in a long note costs no wait. A heading with a competing section link, a block of any other kind, or words that now reach past the block — a new blank line, a list marker, a bracket the rest of the note might answer — draws the whole page again.
...
The heading, the tab and the pane all move to the new name, unsaved typing stays where it was, and every [history step](02-navigation.md#history) standing on that file follows it too. With the [source view](#code-view) open, it stays at the place you were reading rather than going back to line one. A heading the document does write for itself — an XML `<title>`, a message's `Subject`, a [JSON, YAML or INI](#editing-data-files) `title` or `name` key, the title of a [Word file, a deck or an OpenDocument file](01-rendering.md#office-and-opendocument-files) — is not a file name, so pressing it edits that value instead of opening the rename box. The XML and email ones take a caret in the words; a data file's opens the value's own source, the way every other value in one does, and so does an office file's, which opens its own paragraph the way every other line of that file does — except in an OpenDocument spreadsheet or presentation, where the heading is the first sheet's or slide's name and pressing it opens just that name, so typing a new one renames the sheet or slide. A sheet or slide the file gives no name is headed with a word of the app's own, which no bytes of the file hold, so pressing it opens nothing.
...
- **A highlight can cross blocks.** Drag from one paragraph into the next and the selection follows, through headings, code blocks and tables alike — so it can be copied or deleted in one go. A block only becomes typable when you click into it, which is what lets the drag leave it; click into one and it is an editor again, and the page around it goes back to being selectable when you click away. A drag that starts and ends inside one line opens that line for typing wherever you let go, so the words it highlighted can be deleted or typed over straight away.
...
- **A grip beside the row and above the column, and it drags.** Run the pointer over a table and one handle stands at the body row you are level with and one above the column you are in. Move from the cell across the left outline to the dots; they stay beside that row as you cross. Drag the outline itself to resize. Take the row grip and carry it down or up: the row lifts and follows the pointer, the rows it passes step aside to open the place it will land, and letting go drops it there. The column grip does the same sideways, and the columns it crosses step aside for it. The space they open inside the table accepts the drop. A release anywhere outside that table cancels, leaving it exactly as it was, so a drag that wanders into the document never moves a row out of the table. The dots mean that one thing and nothing else — a press that never becomes a drag does nothing. The header row has no grip of its own — the pipes are keyed off it.
...
- **Everything else is on a right-click.** Right-click any cell of an open table and its menu adds a row above or below, deletes the row, adds a column left or right, deletes the column, aligns it and sorts by it — all acting on the cell you right-clicked — with [the five that say how that cell gets its value](#a-table-that-adds-itself-up) under them.
...
- **Copy as CSV.** The row above a table carries a copy button just before the [full-window opener](#inline-editing-the-reading-view), at the table's right edge: it copies the table as a spreadsheet reads it, quoting a cell only where it holds a comma, a quote or a line break. It only reads, so it is there on a locked page too, and it copies the words as they are drawn rather than the Markdown behind them.
- **Markdown only, and only unlocked.** The grips, the keys and the menu appear on a safe Markdown table with the [reading padlock](#the-padlock) lifted. A table the app could not write back keeps the editor it already had, and a locked page keeps everything but Copy as CSV out of the way.
...
- **A row of totals holds its place in a sort.** [Sorting a column](#editing-a-table) moves every body row except one whose cells are all kept numbers, so a totals row cannot be carried into the middle of the data. A row of ordinary data with one kept number in it is not one of these and moves like any other.
- **Writing the rule by hand.** The line is `<!-- TBLFM: ... -->` under the table, in the syntax org-mode uses: `$4=$2*$3` gives every row's fourth column the product of its second and third, and `@>$4=sum(@2..@-1)` gives the last row's fourth column the sum of the body rows above it. `$n` is a column, `@n` a row with the header as `@1`, `@>` the last row and `@-1` one back from it; a range between two of them reads the cells between. It has `+ - * /`, parentheses, a leading minus, and `sum`, `average` (or `avg` / `mean`), `min`, `max` and `count`. A cell whose inputs are not all numbers is left as you wrote it, an answer carries as many decimal places as the widest number it read, and a line the app cannot parse is ignored whole — the table stays an ordinary table rather than half computing.
...
- **The bar waits for you to let go.** While the button is held to drag out a highlight, no bar stands over the page, so a drag that wanders up into the space between blocks or out into the margin keeps only the words you dragged; the bar rises over them on the release.
- **The bar goes under the words where there is no room above them**, with its point on its top edge instead of its bottom, so a highlight on the first line of a page is not handed a bar drawn behind the tab strip. It hangs off the last line of the highlight there, rather than the first, so the rest of what you chose stays visible. It is the same answer the bar already gives sideways: it stops short of either edge of the page and slides its point along to keep pointing at the words.
...
- **On leaftext.com and Emptyguru, the source is for reading.** The code view opens there, but its padlock is absent and typing cannot change the site's copy. The reading view's padlock starts shut on every visit and opening it lasts only for that visit. Opening it on leaftext.com's front page keeps the page exactly as it was laid out: the title, the line under it, each card's words and the install line take a caret where they are drawn, and what you type is written into the README that Save hands you. Everything else on that page — the buttons, the comparison, the card titles and the links at the foot — stays as it is.
...
On leaftext.com and Emptyguru, the same button shows the published document's source read-only. The reading button returns to the page held beneath the source view, keeping its reading edits and position.
...
- On leaftext.com and Emptyguru, every page opens locked. Open the padlock and you can type in the page to try the editor; the edits stay in that tab. **Save** appears once you have typed and downloads the edited file under its own name, and a refresh brings back the published words under a closed padlock. `Ctrl+S` sends no save request for a clean page.
...
- A **[Word, Excel, PowerPoint or OpenDocument file](01-rendering.md#office-and-opendocument-files)** takes the whole archive with it when it reloads, not only the part on screen, so a save after an outside change writes the file that arrived rather than the one it replaced. An **[EPUB book](01-rendering.md#epub-books)** is read whole again, since nothing in one is edited.
...
The [code view](#code-view) edits every format Leaftext opens, as the whole-file source editor: Markdown (`.md`, `.markdown`, `.mdown`, `.mdc`), [HTML](01-rendering.md#html-files) (`.html`, `.htm`), [XML](01-rendering.md#xml) (`.xml`), [JSON and YAML](01-rendering.md#data-files-json-and-yaml) (`.json`, `.yaml`, `.yml`), [plain text](01-rendering.md#plain-text-files) (`.txt`), [INI](01-rendering.md#ini-files) (`.ini`), [email](01-rendering.md#email-eml) (`.eml`, `.mht`, `.mhtml`), and every [source file](01-rendering.md#source-files). A [Word, Excel, PowerPoint or OpenDocument file](01-rendering.md#office-and-opendocument-files) (`.docx`, `.docm`, `.xlsx`, `.xlsm`, `.pptx`, `.pptm`, `.odt`, `.ods`, `.odp`) is the one exception to *whole-file*: it is a zip rather than one text file, so the code view shows the XML of the part the page is anchored to — the first sheet or slide, which is also the only one typed into in the reading view, where its blocks wear a thin accent bar while the padlock is open — and a save rewrites that part and copies every other one byte for byte. An [EPUB book](01-rendering.md#epub-books) (`.epub`) is the one format the code view does not open at all: a book is made of a source file per chapter, so there is no single source to show, and the button says so rather than opening an editor over invented text.
...
| A safe Markdown table | Type in any cell, drag a row or column by its grip, and restructure it from its right-click menu: rows and columns added and deleted, a column aligned or sorted, the whole table copied as CSV. [Editing a table](#editing-a-table) |
| Markdown blocks that cannot round-trip losslessly | Edit their exact source in place. [Block gutter](#the-block-gutter). A checkbox in such a table is the exception: it ticks where it sits, whatever else the table holds |
...
| A block of raw HTML the page does draw — a `<figure>` holding its caption, a wrapper written with its content inside it | Read-only where it is drawn, because the file gives that whole block one range and the markup inside it none; edited in the code view. Every other block in the note stays editable around it, and a wrapper written with a blank line before and after its content leaves the Markdown inside it editable as ordinary blocks |
...
| Any block in a workbook or deck beyond the first sheet or slide | Read-only in the page; edited in the code view, which shows the part the page is anchored to. With the padlock open, the blocks that can be typed in wear a thin accent bar on their leading edge, and the app says once as the padlock opens that the rest is edited in the source view |
| Every part of an [EPUB book](01-rendering.md#epub-books) | Read-only. A book spans a source file per chapter, so no block can say which file its bytes would be spliced into, and the source button says a book is made of several source files |
