...
| [Typing on a value inside an XML tag](#inline-editing-the-reading-view) | Press a value written inside a tag — in the element's own list, in a table cell, or as one of several drawn together — and type on it where it is drawn; only what sits between the quotes is written, and the quote that closes it is refused |
...
| [Highlighting](#code-view) | The source is colored in the active [theme](06-themes.md)'s syntax colors — Markdown, HTML, XML, YAML and JSON; raw [email](01-rendering.md#email-eml) shows as uncolored text |
...
| A value written inside an XML tag | Type on the words where they are drawn; only the bytes between that value's quotes are written, and the quote that closes it is refused |
