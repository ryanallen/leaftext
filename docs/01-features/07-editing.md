...
- **A cell of an XML table types on its words as well.** A table drawn from a run of records — a sitemap's addresses and dates, a feed's entries — takes a caret in any cell whose words are one element's own bytes, and what you type is written into that element alone: no other row of the file moves. Where a record holds several elements of one name the cell draws them joined by a comma, and each value takes a caret of its own: type on one and the others stay exactly as they were, while the comma between them belongs to neither and answers a press with nothing. A cell drawn from a value inside a tag and a column a record has nothing for are not the file's own words, so those answer a press with nothing too.
