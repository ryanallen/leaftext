...
![The library pane open beside a document: the vault switcher at top left, the folder breadcrumb beneath it, and a file list showing three folders first and then the files beside them](../../imgs/library.png)
...
The button at the left of the breadcrumb opens the vault switcher: pointing at it with a mouse is enough, and pressing it holds the list open until you press it again. Touch and pen open it on the press alone. Move the pointer off the button and the list, and the list fades away after half a second — the small gap between the two belongs to the list, so travelling down into it never loses it, and coming back before it has gone brings it whole again. The same button appears over the start screen whenever a vault exists, so switching to Library never leaves you there. What it wears says what you are in: **this machine** for the whole library, a **box** for a vault whose files only live here, and a **cloud** when saving in that vault also reaches somewhere else — [GitHub](#github-sync), or a [cloud folder](#your-cloud-is-already-a-folder). Its mark stays the regular weight and sits apart from its name.
...
The pane lists Markdown (`.md`, `.markdown`, `.mdown`, `.mdc` — a [Cursor](https://cursor.com) project rule is Markdown with a [field block](07-editing.md#the-fields-at-the-top-of-a-note) at the top), [HTML](01-rendering.md#html-files) (`.html`, `.htm`), [XML](01-rendering.md#xml) (`.xml`), [JSON and YAML](01-rendering.md#data-files-json-and-yaml) (`.json`, `.yaml`, `.yml`), [plain text](01-rendering.md#plain-text-files) (`.txt`), [INI](01-rendering.md#ini-files) (`.ini`), [email](01-rendering.md#email-eml) (`.eml`, `.mht`, `.mhtml`), [Word, Excel, PowerPoint and OpenDocument](01-rendering.md#office-and-opendocument-files) (`.docx`, `.docm`, `.xlsx`, `.xlsm`, `.pptx`, `.pptm`, `.odt`, `.ods`, `.odp`), and [EPUB books](01-rendering.md#epub-books) (`.epub`). [Source files](01-rendering.md#source-files) open when named without becoming library documents, vault-search text, graph nodes, or Previous/Next pages. A file with no ending at all — `CHANGELOG`, `LICENSE`, `AUTHORS`, `.gitignore` — is listed too when its opening bytes are text, and is searched, mapped and paged like any note; one that holds a binary, or nothing at all, is left out, and nothing inside a `.git` folder is ever read. Anything else is left alone. Where that empties the pane, the line in its place says how many files are in the folder, so a folder of pictures reads as a folder of pictures rather than a folder Leaftext lost.
...
![All-files search results hanging under the find bar: a filename match ranked at the top, then content matches showing the document name and highlighted terms, with the pane's outline still behind them](../../imgs/search.png)
...
- **Coming back to the map shows it at once.** Leaving it for another view keeps the drawing, paused, so switching back is the same picture at the same corner with no wait — a layout that was still settling picks up where it stopped. It is drawn again only when something could have changed it: a different graph size, a document whose map is a different map, a vault switch, or a file behind it being edited, saved or changed on disk.
...
![A vault's settings panel showing the connected GitHub repository address with Sync, Sync automatically and Change repo under it, and the sync button at the end of the breadcrumb carrying a count of changes waiting to be pushed](../../imgs/github-sync.png)
...
**Sync** commits everything changed, asks GitHub whether it holds anything the vault lacks, and pushes. Only when it does is the vault rebased onto it, after committing anything written since the first commit — so a file another program or a second window writes while a sync runs never stops the push. Commit messages describe the change — `Update README.md`, or `Update 4 files` — and carry no mention of the app.

GitHub sync starts manually unless this vault's settings has **Sync automatically** turned on. The switch belongs to one vault and starts off, so private notes can wait for a press while another vault sends itself. An automatic sync waits for the vault to stop changing: it starts ten seconds after the last change, so a run of saves — yours, or an agent's, or another program's — goes to GitHub as one sync rather than one per file. A vault that never goes quiet is sent anyway after two minutes. Pressing Sync during that wait starts at once. Automatic sync uses the same Sync turn, count and result growl as the button. After a failure, later saves leave the header's Sync button waiting instead of trying again, the switch stays on, and the panel says **Automatic sync stopped. Press Sync to start it again.** The button says so too, for as long as it lasts: it turns red and reads **Stopped** before its count, its tooltip carries what went wrong, and the failure's growl adds that automatic sync is off until a sync works. Pressing Sync or completing another GitHub action allows automatic sync to start again. **A sync is never given up on for taking too long.** Sending a large vault the first time can run for as long as the connection needs; what stops a transfer is going quiet, and it is stopped only after two minutes with nothing arriving, which the panel and the growl say in those words rather than blaming the network. Every failed sync also writes one line into the [journal](05-settings.md#journal) naming the vault, the step, how git ended and everything git said, so a sync that stopped while you were away leaves something to read; a sync that worked writes nothing.

A **sync button appears at the end of the vault's breadcrumb** whenever there is work that has not reached GitHub, carrying the count. It spins for as long as the sync is really running — opening the vault's settings mid-sync leaves it spinning and the panel's rows dimmed, so a second sync cannot start over the first — and a vault too large to stage quickly is never cut off part way, since stopping git there would leave the repository locked against every later sync. **Resting on it while it spins shows how far along the upload is**, in a bar under the breadcrumb reading the count and the percentage git itself is reporting — `283 of 827 · 34%`; a keyboard focus opens the same bar, a press on the spinning button still does nothing, and a step with nothing to count draws no bar. It fades out still spinning — more slowly under [Reduce Motion](05-settings.md#reduce-motion), which slows every spinner rather than stopping it — and a growl in the corner says where the push landed. Where GitHub held commits the vault lacked, that growl and the panel's note count them at the end of the same sentence — **Nothing to send; 3 came down.** — which is the only record you get that the vault changed under you, since the panel's standing above it reads as up to date by then. It is absent when there is nothing to send.
...
- The vault's in-memory text is patched for the one file that changed, so [search](#search), the [graph](#graph) and the completion menu under the [filter box](#filtering) stay current without re-reading the vault. Saving the note you are reading takes the same path, so the words you just wrote are findable at once rather than after the vault has been read again. So does a checkbox you tick in the page, in a list or inside a table, which writes the file straight away and with no press of Save behind it. Only a document whose text actually moved counts: a vault is a folder you work in, and git writing to itself, a saved image or an editor's temp file are not changes to your documents.
