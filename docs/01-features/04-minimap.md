...
> [!NOTE]
> The thumbnail is a second, scaled-down layout, so it cannot exist until the document itself has been laid out. Until it does the rail shows a small spinner rather than an empty lane — on a large document that build is a visible wait, and a blank rail beside a finished page reads as one that failed rather than one still working. The rail goes back to the spinner while [Mermaid diagrams](01-rendering.md#mermaid-diagrams) are being drawn, since they land seconds after the words and a thumbnail taken before them is a picture of a page that is about to be replaced.
