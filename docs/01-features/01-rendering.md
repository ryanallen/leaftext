...
Left of that group sits one more button, and a shut padlock keeps it: it saves the diagram as a file of its own. **Markdown** writes the Mermaid text in a fenced block; **PNG** writes the drawing as a picture at twice its size, on your theme's page color and with room around it. Either way the app asks where the file goes, and the document you are reading is not touched — the same two files [the flowchart editor](07-editing.md#export) writes, on every kind of diagram rather than only the ones it can draw. The full-window view carries the same button.
