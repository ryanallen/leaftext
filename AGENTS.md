...
| [`priority`](.agents/skills/priority/SKILL.md) | deciding what to build next. Ranks every live ticket into one running order and writes it to `../docs/plans/`, dated — wrong today first, then what other tickets are waiting on, then cost. |
...
| [`build`](.agents/skills/build/SKILL.md) | building a ticket somebody already planned. Runs `refine` first if nothing has dated the file, then works the phases in order, ticks the boxes, and when the last one lands moves the file to `../docs/done/` and updates the index and the running order in the same pass. |
