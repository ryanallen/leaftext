...
- **One front end, two hosts — and a command with no browser line does not ship.** Everything a reader touches is the same page and the same script; what differs is who answers it, so every change to the app is two changes and only one of them used to be checked. `IpcCommand` is the one typed list of what the page may send, and every arm of it carries a written line in the browser's own host — answered, refused on purpose with the reason, or not yet with the ticket that owns it. `just check-web-commands` fails on an arm with no line, on a line naming no arm, and on a command the front end sends by name that neither host has. Write the line in the same edit as the command, because an unanswered one is not a control that does nothing: the page raises a waiting state before it sends and clears it only when the answer arrives, so it waits for ever. `check-shell` boots that host over a stand-in module offline, and a tagged release builds the modules for `wasm32`, where a break nothing local can see shows up.
...
Needs `rustup`, `just`, `node`. `/check` is the gate before handing work back: `just verify` (fmt, check, test, vendor + theme + token + icon + gallery + docs drift, no hand-written values, US spelling, no hard-wrapped paragraph, no diagram drawn with box characters, every Markdown file has something keeping it true, every relative `.md` link in both trees opens a file, no shipped plan left in a live folder, every path the published pages fetch is a file, front-end boot, identity, the two hooks) with a test pass in front of it. `just check` / `test` / `format` / `check-shell` run individually.
