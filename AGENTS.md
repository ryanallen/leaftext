...
| [`build`](.agents/skills/build/SKILL.md) | building a ticket somebody already planned. Runs `design` first if nothing has dated the file, then works the phases in order, ticks the boxes, and when the last one lands moves the file into a subject folder under `../docs/done/`, moves its row out of the running order into `../docs/done/PLAN.md`, and updates the index in the same pass. |
