...
- `theme.rs` — token contract, Primer/Dracula tables, CSS compiler. The stylesheet itself is `assets/reading.css`; `reading_mode_css()` prepends the compiled tokens to it. **Mermaid diagrams take those same tokens** — `decorate.js` maps them over mermaid's own light/dark theme and measures the ink for text on a fill we chose — so a theme file says nothing about diagrams and a new family themes them for free. Never a per-theme diagram palette. **The `cScale` categorical scale is mermaid's and cannot be ours**: `base` darkens every entry after the override lands, which is how v0.1.423 shipped near-black boxes with near-black labels
