...
- `gate-touched.mjs` — writes down every file this session changes, so a release stages its own work and leaves another session's alone, and samples the build ticket's boxes after each edit.
