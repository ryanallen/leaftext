...
**A hairline window row keeps a filled pack's own drawing only where the worn result already lands on that weight.** A filled glyph takes no stamp, so nothing can bring it to the row's weight after the fact — and these four marks sit beside the platform's own chrome, where the hairline is a match to somebody else's buttons rather than a house style. So the measurement decides: Remix's minimize measures 0.996 px at the 12 px it is worn at and keeps its own three drawings, while Phosphor's measured 0.753 px, so its minimize, maximize and close say `leaftext` and take the app's measured marks the way its restore already did. Everywhere else a filled pack draws its own shape at whatever weight that shape carries, which is what a filled pack is for.

**A hairline window row publishes one more mask per size the window wears it at**, `--lt-icon-<name>-chip` for the Windows chip and `--lt-icon-<name>-dot` for the Mac dot, each the same drawing moved onto that size's pixel grid. A one-pixel line centered on a pixel boundary draws at half its ink in each of two rows, so the same mark reads paler than the palette and folder beside it while carrying the same color; the move is measured per drawing and per size, because a drawing held at 12 px is not held at 8. It rides on the mask's own viewBox origin, so nothing on disk moves and the ordinary mask every other control wears is unchanged. Only these four rows take one: every other drawing is worn at eleven sizes and no one move holds it to all of them. `just check-icon-grid` measures the masks that ship and refuses one a move would hold, naming rather than refusing a diagonal, a curve, a filled drawing and a square whose two edges are a fractional number of pixels apart.
...
| window-minimize | window-minimize.svg | leaftext | hairline | — | window-minimize | minus | minus | leaftext | subtract | leaftext | minus | The window's own minimize button. Drawn on a 12px grid at a hairline, because it sits in the title bar beside the platform's own chrome. |
| window-maximize | window-maximize.svg | leaftext | hairline | — | window-maximize | square | square | leaftext | checkbox-blank | leaftext | leaftext | Maximize. Same grid. |
...
| window-close | window-close.svg | leaftext | hairline | — | window-close | x | x | x | close | leaftext | x-mark | The window's close cross. |
...
| footnote | asterisk.svg | lucide | regular | — | footnote | leaftext | leaftext | leaftext | leaftext | leaftext | leaftext | Annotate, in the selection bar over highlighted words. An asterisk rather than a speech bubble, so the bubble the block gutter already wears keeps meaning an HTML comment. |
