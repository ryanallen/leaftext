...
| Settings rows | settings | `settings.js`, `dom.js`, `updater.js`, `app-shell.html` | `<div class="settings-panel"><label class="setting-control setting-control-inline"><input type="checkbox" checked><span class="setting-label">Show minimap</span><span class="setting-help">A scrollable overview of the page, on wider windows.</span></label></div>` | setting-control setting-label setting-help |
...
| Spinner | lt-spinner | `app-shell.html`, `glossary.js`, `minimap.js`, `lib.rs` | `<div class="settings-footer" style="justify-content:flex-start"><span class="lt-spinner settings-spinner"></span></div><div class="glossary-sheet-waiting"><div class="lt-spinner glossary-sheet-spinner"></div></div>` | — |
| Icon | lt-icon | everything, drawn by `icons.css` | `<a class="jump" href="#icons"><span class="lt-icon lt-icon-leaf"></span>All 53 icons, on the Icons tab</a>` | — |
...
| Empty state | empty-state | `render-document.js` | `<div class="empty-state" style="padding:8px 0"><div class="empty-subtitle">Nothing open</div><div class="empty-description">Open a document to start.</div></div>` | empty-actions empty-description empty-help empty-subtitle primary-new primary-open kicker recent |
