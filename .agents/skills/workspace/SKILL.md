...
`plan-open` hands back a copy of `../docs/PLAN.md` and holds the file; edit that copy, then `plan-close` writes it back and gives the claim up. **A lock file on its own binds nobody**, which is why the pair does the reading and the writing rather than merely marking the file: a second session asking for it inside the first one's edit waits rather than reading rows somebody is halfway through, and a copy taken before another session's row landed is refused with what to do — open it again and redo the row — rather than written back over it. A claim a killed run left behind goes stale in two minutes and the next session takes it over. Every skill that writes a row or a status names this: [`/pm`](../pm/SKILL.md), [`/design`](../design/SKILL.md), [`/dev`](../dev/SKILL.md), [`/git-release`](../git-release/SKILL.md) and [`/done`](../done/SKILL.md).
