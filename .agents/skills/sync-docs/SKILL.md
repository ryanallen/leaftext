...
- **A paragraph is one line.** Never hard-wrap. The renderer reflows, GitHub reflows, every editor reflows — a wrap only costs, on every edit after it. `just check-wrapping` fails on one and names the file; `--fix` joins them, in Markdown and in `.rs`/`.js` comments alike — but it skips `src/assets/`, so a wrapped comment in a front-end fragment or in `reading.css` is found by reading, never by the check. A break doing real work keeps two trailing spaces, or the file carries `<!-- keep-wrapping -->`. **This applies to every file the sweep in step 7 reaches, not only the page being edited** — a wrapped file is fixed where it is found.
