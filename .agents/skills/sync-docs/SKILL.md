...
| **a skill copied for sharing** (`../docs/learn/ticket-workflow-medium/skills/`) | The one part of `learn/` that is this repo's own writing: the workflow article is handed on with an exact copy of every skill it cites, so the copy is the reader's evidence and a drifted one teaches a rule that was retired. Never edited in place — `just check-learn-snapshots` fails on one that differs and `node scripts/check-learn-snapshots.mjs --fix` rewrites every copy from its source |
