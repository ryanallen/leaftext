...
This is the only skill that commits, tags, pushes, or changes the version. It commits twice: **`just land <message>` first, before anything else**, then `/sync-docs` and `/code-comments`, then the release itself. Never add assistant identity to a commit.

**It runs no complete gate.** The code it ships was proved at the end of its build and has not moved since, so running the suite here proves the same bytes a second time — which is the whole cost this pass used to carry. What this pass writes itself is pages and comments, and each has its own narrow proof: the document checks for a page, the Rust documentation build for a comment. The app is not recompiled and the tests are not run again.
...
### 5. Sync the pages and trim the comments

There is no handoff and nothing to submit: the code, the plan tree and the release are all in front of you. Land what is in the tree, run `/sync-docs` and `/code-comments` over what this pass ships, land again, then release from here. Neither of those runs the complete gate.
