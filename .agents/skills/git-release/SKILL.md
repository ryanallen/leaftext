...
**Every run pushes.** The landing above has already happened, so the classification below decides one thing: whether a version and a tag follow. "Stop" means stop cutting a release, never stop before the landing — a run that ends with files still in the checkout skipped it.
