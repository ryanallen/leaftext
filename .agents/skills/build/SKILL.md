...
4. **The plan in `../docs/plans/`**: strike the row through, mark it `Done <date>`, and say what the build found — what the plan had wrong, and what changed shape. Then move the **"Where this stands"** paragraph and its "next up" pointer on to the next open row. Rows are numbered and the sections below the tables cite them by number, so fix any reference that named this one.
