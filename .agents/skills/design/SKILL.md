...
**Write it under the claim, never straight over the file.** `node scripts/agent-workspace.mjs plan-open` hands back a copy to edit, `plan-close` writes it back under the claim, and a copy taken before another session's row is refused rather than written over it — kept where it is, so the row is redone from it. Edit the copy it names; [`/pm`](../pm/SKILL.md) holds the reason.
