...
**A pick ends here.** Where the retiring row sits in `## Picked by the owner`, its row leaves that band with the rest of them, and the band goes heading and all where it holds nothing else — a pick is the owner saying build this now, and a ticket that has shipped is not something anybody can build. That is the one time a pick is cleared without the owner's word, because the work it named is gone; taking one out for any other reason is theirs alone.
