---
name: dev
description: Build a ticket from its path under ../docs/features/, ../docs/refactor/ or ../docs/fixes/. Runs /design first when nothing has dated the file, works the phases in order, ticks each box in the same edit as the code with its test, runs /check per phase, drives what it can reach in the running window, and stops at the owner's own box — closing a ticket is /pre-release's, on the owner's word. Use when the user says "build this", "dev this", "work that ticket", "do the plan", or hands over a ticket path to be built rather than scoped.
...
- **Never invent interface.** A control the ticket's `## What it looks like` section does not draw is a control that does not get built — not a button, not an input, not a panel, not a colored mark. No section and the phases touch the window? Then **draw it and get a yes before a line of front-end code**: sketch what you mean to add as HTML and photograph it with `node scripts/wireframe.mjs` into the ticket — a picture, never box characters: `just check-ascii-art` fails on a `┌` anywhere in the tree — put that drawing in the message, give two or three options where there is a real choice with one marked the pick, and wait. Write the answer into the ticket as that section, then build only what it shows. v0.1.479's filter work shipped a second search box, a `?` button and a popup nobody had asked for; all three came out again in the same session, along with the plan that had let them in.
- **Build to the flow diagram, and redraw it the moment the code disagrees.** A ticket explaining a mechanism carries one in `## How it is built` — the hops, the branch, the line between the page and the host. Read it before the phase and follow it: it is the fastest way to know which piece answers what, and a hop it shows that the code does not have is a box, not a suggestion. When building changes the real flow — an edge that turned out to go the other way, a step that was not needed — **edit the block in the same edit as the box**, exactly as a box is ticked, and say what moved in the record. A shipped ticket whose picture describes a flow the code never had is worse than one with no picture, because the next reader trusts it.
- **Cheapest right answer first: use the control that is already there.** A second box beside an existing one is nearly always the wrong build, and "why can it not use the same input" is the question to answer before writing any of it.
