...
## 🛑 Only the owner starts a build

**This skill runs only when the message being answered right now was typed by the owner and carries `/dev` or `$dev`; nothing else authorizes one.**

## Process

### 1. The owner's words and pictures go in the ticket first, before anything else in this turn

**Every message that calls this skill is copied into the ticket whole and word for word, and every image it carries is saved into `../docs/imgs/` and embedded, before anything else in the turn happens.** Every message: the one that starts the build as much as the tenth round of fixes, one that names a ticket path and one that just says what is wrong, one sentence and one of ten paragraphs. Not after the cause is read, not after a file is opened, not after one line of code, not "once I know which phase it belongs to" — the very first act of the turn is the round written into the ticket, and the turn goes no further until it is on disk.

**Word for word means the owner's own characters**, first to last, in a quoted block under a bold line naming the round: their spelling, their punctuation, their swearing, the clause about something else entirely, the part that repeats what they said last time. Never a summary, never a cleaned-up version, never the half that is about the code. A message paraphrased is a message the owner cannot check, and the ticket is the only place they can check it.

**Every image is saved to disk and embedded, never left in the transcript.** The host hands an attached or pasted picture over as a file; copy those bytes into `../docs/imgs/` with a file-safe tool — named after the ticket, numbered `-2`, `-3` for the later ones — and embed it in the round under the line it is evidence for, with alt text saying what is in the frame. A supplied screenshot is that state's picture and is never redrawn as a wireframe. Where the bytes genuinely cannot be reached, the round says which image is missing and what it showed, in the ticket rather than in the reply.

**Then one box per thing the owner said they will look for.** Every clause is its own box, the asides included, written into the phase it belongs to before a line of code is written. Then [`/design`](../design/SKILL.md) over that round, so the fix is read against the code before it is built. Only then does the turn go on to the step below and build it.

**Reading the code first is the failure this step exists to stop.** A session that opens a file before it writes the round has already decided what the owner meant, and the owner is left watching a build happen against a ticket that says nothing about what they just asked for — which is the one thing they cannot check, because the ticket is where they read it. A round taken from memory also loses whichever ask came last in the sentence, every time, and a picture left in the transcript is gone the moment the session ends.

`## Refining what was built goes in the ticket, every round, unasked` below holds the shape; this step holds when it happens, which is first.
...
**Tick the ticket file, never a window.** After the code and test finish one box, change that box's `- [ ]` to `- [x]` with the host's file-safe edit tool, in the same edit that lands the code and test. `leaftext_doc` and `leaftext_toggle_task` are forbidden for a box and there is no fallback that opens a window: reading a ticket through the app brings its page to the front, and the toggle refuses a document that is not already there, so every tick would take the page and the keyboard away from whatever the owner is reading. Recording progress touches no running app at all, which is also why a build ticks while the tree does not compile.
...
- **Every owner prompt is copied into the round whole and every picture saved inline, before the fix rather than in the same edit as it** — step 1 holds the timing, and it is first in the turn without exception. [`/ticket`](../ticket/SKILL.md)'s rule holds here unchanged — `../docs/imgs/`, named after the ticket with `-2`, `-3` for the later ones, embedded under the line it is evidence for with alt text saying what is in the frame. A supplied screenshot is used as that state’s picture, not redrawn.
