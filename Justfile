...
bundle-icon-audit:
    node scripts/bundle-icon-audit.mjs

# Fail if the audit has drifted from design/icons.md, or a sheet was never
# photographed. Offline and parser-only: no browser, so this can sit in `verify`.
check-icon-audit:
    node scripts/bundle-icon-audit.mjs --check
...
# Write the giveaway archive from the system rows in the folder's own file table.
bundle-giveaway:
    node scripts/bundle-giveaway.mjs

# Fail when the giveaway archive no longer matches those rows entry by entry.
check-giveaway:
    node scripts/bundle-giveaway.mjs --check
...
# Hold the build skill to Leaftext's guarded task action for every finished ticket box.
check-dev-task-toggle:
    node scripts/check-dev-task-toggle.mjs --check

# Hold the complete suite to one automatic run a ticket, at the end of its build. Every
# other pass proves what it writes with the narrow checks about it.
check-suite-callers:
    node scripts/check-suite-callers.mjs --check
...
# Open a PDF the app wrote and say whether it printed drawings or waiting frames: a
# diagram the page never drew goes onto the paper as its own source in invisible
# ink, and this counts those lines sheet by sheet. Non-zero where there is one.
read-export file:
    node scripts/read-export.mjs {{ file }}

# Prove that reader on two files it writes itself, so a phase about exporting is
# ticked against the file a reader is handed and never against the page alone.
check-read-export:
    node scripts/read-export.mjs --check
