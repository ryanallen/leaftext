#!/usr/bin/env node
// PreToolUse hook. Writes down every file this session's edits touch, so a release can stage its own work and leave another session's alone.
//
//   node scripts/gate-touched.mjs           the hook payload on stdin
...
//
// Only the edit tools are recorded, which is the point rather than a gap: `Cargo.lock`, the bundled stylesheets and the discovery files are written by commands, so no session's record claims them and the subtraction keeps them for whoever is releasing.
//
...
function selfTest() {
  ...
  for (const name of ['Bash', 'PowerShell', 'Read', 'TodoWrite', 'Grep']) {
    if (EDIT_TOOLS.test(name)) fails.push(`${name} was read as an edit, so a command's own output would be subtracted from a release`);
  }
  ...
  console.log('gate-touched: ok (an edit inside the checkout is written down under its own session, one outside it is dropped, a record grows rather than being replaced, no session id records nothing and subtracts nothing, and a day-old record is swept)');
}
