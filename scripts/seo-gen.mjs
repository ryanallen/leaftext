...
// The address advertised for a file. A doc page gets its raw .md rather than its `#/route`: a fragment never reaches the server, so 18 routes all answered as one request for /docs/. The router still serves the routes people share.
...
// ---- build ----------------------------------------------------------------
// Every file, by name, as a string. Nothing is written here, so `check-site.mjs` can run the same code and hold the committed files to it — the set was regenerated by hand and nothing noticed when it was not.

export function discoveryFiles() {
  const docsMd = existsSync(join(ROOT, 'docs')) ? findMarkdown(join(ROOT, 'docs')) : [];
  const pages = ['README.md', ...docsMd].map((relPath) => ({
    relPath,
  ...
  // The page address and its raw source. They are the same file for every doc page, so only the two index pages contribute two.
  const urls = [];
  ...
  urls.push({ loc: GALLERY.page, lastmod: lastmod(GALLERY.relPath) });
  const seen = new Set();
  const sitemap =
  ...
  return {
    ...
    files: {
      'robots.txt': robots,
      'sitemap.xml': sitemap,
      'sitemap-md.txt': pages.map((p) => p.md).join('\n') + '\n',
      'llms.txt': llms,
...
// ---- write ----------------------------------------------------------------
// Only when run as a program. Imported, this file writes nothing.

if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const { pages, files } = discoveryFiles();
  for (const [name, body] of Object.entries(files)) writeFileSync(join(ROOT, name), body);
  console.log(`seo-gen: ${pages} pages for ${ORIGIN}`);
  console.log('  ' + Object.keys(files).join(', ') + ' written.');
